...
add_action( 'wp_ajax_ampforwp_get_licence_activate_update', 'amp_pbc_get_extension_licence_activate_update' );

function amp_pbc_get_extension_licence_activate_update(){
        ...
        if( isset($selectedOption['amp-license']) && "" != $selectedOption['amp-license']){
            // data to send in our API request
                    ...
                    if ( false === $license_data->success ) {
                        switch( $license_data->error ) {
                            case 'expired' :
                                ...
                                $message .= "<a href='".$store_url."/checkout-2/?edd_license_key='".$license.">Renew Link</a>";
                                break;
                ...
                if ( ! empty( $message ) ) {
                    ...
                    $limit = amp_pbc_set_extension_plugin_limit( true, $license_data, $ampforwp_license_activate);
...
function amp_pbc_extension_deactivate_license() {
    if(!wp_verify_nonce( $_REQUEST['verify_nonce'], 'verify_extension' ) ) {
        echo json_encode(array("status"=>300,"message"=>'Request not valid'));
        die;
    }
...
add_action( 'wp_ajax_ampforwp_deactivate_license', 'amp_pbc_extension_deactivate_license' );


function amp_pbc_set_extension_plugin_limit( $force=false, $license_data='', $data) {
