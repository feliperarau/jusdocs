 ...
 */
class AMP_PB_Style_Sanitizer extends AMP_PB_Base_Sanitizer {
	...
	private function get_used_class_names() {
		if ( empty( $this->used_class_names ) ) {
			...
			foreach ( $this->xpath->query( '//*/@' . AMP_PB_DOM_Utils::get_amp_bind_placeholder_prefix() . 'class' ) as $bound_class_attribute ) {
	...
	private function fetch_external_stylesheet( $url ) {
		...
		$contents  = amp_pbc_get_file_transient( $cache_key );
		if ( false === $contents ) {
			$r = wp_remote_get( $url );
	...
	private function process_stylesheet( $stylesheet, $options = array() ) {
		...
		if ( wp_using_ext_object_cache() ) {
			...
			$parsed = amp_pbc_get_file_transient( $cache_key . $cache_group );
		} 
		...
		if ( ! $parsed || ! isset( $parsed['stylesheet'] ) || ! is_array( $parsed['stylesheet'] ) ) {
			...
			if ( wp_using_ext_object_cache() ) {
				...
				amp_pbc_set_file_transient( $cache_key . $cache_group, $parsed, MONTH_IN_SECONDS );
			} 
		}
	...
	private function transform_important_qualifiers( RuleSet $ruleset, CSSList $css_list ) {
		...
		$important_ruleset->setSelectors( array_map(
			/**
			 * Modify selectors to be more specific to rougAMP_PB_Style_Sanitizerhly match the effect of !important.
			 *
			...
			function( Selector $old_selector ) {
				...
				$specificity_multiplier = AMP_PB_Style_Sanitizer::INLINE_SPECIFICITY_MULTIPLIER + 1 + floor( $old_selector->getSpecificity() / 100 );
