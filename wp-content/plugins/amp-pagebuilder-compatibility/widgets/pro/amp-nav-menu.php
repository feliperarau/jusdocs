...
class Amp_Nav_Menu extends Base_Widget {
	...
	public function amp_elementor_nav_menu_scripts($data){
		$data['amp_component_scripts']['amp-bind'] = 'https://cdn.ampproject.org/v0/amp-bind-0.1.js';
  		return $data;
	}
	public function amp_pc_ele_nav_menu_script($completeContent){
		preg_match('/<head>(.*?)custom-element="amp-bind"(.*?)<\/script>(.*?)<\/head>/', $completeContent,$matches);
		if(empty($matches) ){
			$completeContent = preg_replace('/<head>(.*?)<\/script>(.*?)<\/head>/', '<head>$1</script><script custom-element="amp-bind" src="https://cdn.ampproject.org/v0/amp-bind-0.1.js" async></script>$2</head>', $completeContent);
		}

$completeContent = preg_replace('/<li class="menu-item(.*?)menu-item-has-children(.*?)"><a href="(.*?)">(.*?)<\/a>/', '<li class="menu-item$1 menu-item-has-children$2"><a href="$3">$4</a><label class="toggle"></label>', $completeContent);

 $completeContent = preg_replace('/<li class="mit(.*?)mit-has-children(.*?)"><a href="(.*?)">(.*?)<\/a>/', '<li class="mit$1 mit-has-children$2"><a href="$3">$4</a><label class="toggle"></label>', $completeContent);

		return $completeContent;
	}
	public function amp_elementor_nav_menu_styles(){
		$inline_styles = '@media only screen and (max-width: 768px) {
				.elementor-top-column .elementor-nav-menu--toggle .elementor-nav-menu--dropdown.elementor-nav-menu__container{
			    top: 33px;
			    width: 425px;
			    left: -350px;
			    transform: scaleY(1);
			...
			.m-menu.elementor-n-m--dropdown.elementor-n-m__container {
			    top: 10vh;
			    width: 100vw;
			    left: -69vw;
			}
			...
			.elementor-top-column .elementor-nav-menu--toggle .elementor-menu-toggle:not(.elementor-active)+.elementor-nav-menu__container {
			    transform: scaleY(0);
			    max-height: 0;
			}
			.m-menu .a-m > li a {
			...
			.m-menu .a-m li.menu-item-has-children>ul>li{
			    border-bottom: 0px solid;
			}
			.elementor-menu-toggle.elementor-active i:before {
			    content: "\e87f";
			}
			.elementor-nav-menu--toggle .elementor-nav-menu--dropdown.elementor-nav-menu__container .toggle:after {
			    color: #818a91;
			}
		}
		.ew-premium-google-reviews .premium-fb-rev-review-inner amp-img{display:block}.ew-button .ebtn{background:#f81}.elementor-n-m--main .elementor-n-m ul{width:16em}.menu-item:hover .amp-sub-menu{display:block}.amp-sub-menu .menu-item:hover ul{display:block}.amp-sub-menu-child.elementor-nav-menu--dropdown{width:auto;left:0;margin-left:208px;margin-top:-45px;min-width:10em;max-width:1000px}ul.elementor-nav-menu .toggle:after{top:24px;right:-1px;cursor:pointer;content:"\f107";font-family:"Font Awesome 5 Free";font-weight:600;font-size:14px;display:inline-block;position:absolute;padding:0;line-height:0;color:#353535;visibility:visible;overflow:unset}.m-menu .a-m .toggle:after{color:#000}';	
        echo $inline_styles;
}
	protected function render() {
		add_filter("ampforwp_the_content_last_filter", [$this,"amp_pc_ele_nav_menu_script"] );
		add_filter('amp_post_template_data', [$this, 'amp_elementor_nav_menu_scripts']);
		add_filter('amp_post_template_css', [$this, 'amp_elementor_nav_menu_styles']);
		...
		add_filter( 'nav_menu_submenu_css_class', [ $this, 'handle_sub_menu_classes' ],10,3 );
		...
		$dropdown_menu_html = apply_filters('ampforwp_menu_content', $dropdown_menu_html);
	    $sanitizer_obj = new \AMPFORWP_Content( $dropdown_menu_html, array(), apply_filters( 'ampforwp_content_sanitizers', array( 'AMP_Img_Sanitizer' => array(), 'AMP_Style_Sanitizer' => array(), ) ) );
	    $sanitized_menu =  $sanitizer_obj->get_amp_content();
		// Remove all our custom filters.
		remove_filter( 'nav_menu_link_attributes', [ $this, 'handle_link_classes' ] );
		remove_filter( 'nav_menu_submenu_css_class', [ $this, 'handle_sub_menu_classes' ],10,3 );
		...
		?>
		<div [class]="visible<?php echo $this->get_id(); ?>?'elementor-menu-toggle elementor-active':'elementor-menu-toggle'" <?php echo $this->get_render_attribute_string( 'menu-toggle' ); ?>>
			<i class="eicon-menu-bar" role="button" tabindex="0" aria-hidden="true" on="tap:AMP.setState({visible<?php echo $this->get_id(); ?>: !visible<?php echo $this->get_id(); ?>})"></i>
			<i class="elementor-active hide" ></i>
			<span class="elementor-screen-only"><?php _e( 'Menu', 'elementor-pro' ); ?></span>
		</div>
		<nav class="m-menu elementor-nav-menu--dropdown elementor-nav-menu__container" role="navigation" aria-hidden="true">
           <?php //$amp_menu = amp_menu_html(true, $menu_args, 'header'); 
           echo $sanitized_menu;
	...
	public function handle_sub_menu_classes( $classes, $args, $depth) {
		foreach ( $classes as $key => $class ) {

		        if ( $class == 'sub-menu' && $depth == 0) {
		            $classes[ $key ] = 'amp-sub-menu';
		        }elseif($class == 'sub-menu' && $depth == 1){
		            $classes[ $key ] = 'amp-sub-menu-child';
		        }
		} 
