...
class Amp_Gallery extends Base_Widget {
	...
	public function amp_elementor_widget_styles(){
		...
		$gap = $settings['gap'];
		$ideal_row_height = $settings['ideal_row_height'];
		$gap_size = isset($gap['size'])? $gap['size']:10;
		$no_of_columns = 4;
		$columns_arr = array();

		//if(3 == ampforwp_get_setting('ampforwp-gallery-design-type')){
			if(isset($settings['columns']) && !empty($settings['columns'])){
				$no_of_columns = $settings['columns'];
			}
		...
		//}
		$inline_styles = "";
		$inline_styles .= '.elementor-element-'.$this->get_id().' .e-gallery-item .ele-gallery-item__content{
			display:none;
		}
		.elementor-element-'.$this->get_id().' .e-gallery-item:hover .ele-gallery-item__content{
			display:flex;
		}';
		
		if($settings['gallery_type'] == 'single' && $settings['gallery_layout'] == 'justified'){
			$inline_styles .= '.elementor-element-'.$this->get_id().' .elementor-gallery__container{
				    	width: 100%;
				    	display: inline-grid; 
				    	flex-wrap: wrap; 
				    	grid-template-columns: '.implode(' ', $columns_arr).';
				    	grid-gap: '.$gap_size.'px '.$gap_size.'px;
				    }';
		}
		/*
		.elementor .elementor-element-'.$this->get_id().' .elementor-gallery__container amp-img{ 
				     	height: '.$ideal_row_height['size'].'px;
				     	position: relative;
				     	float: none;
				    }
		*/
		if($settings['gallery_type'] == 'multiple'){
			$inline_styles .= '.elementor .elementor-element-'.$this->get_id().' .elementor-gallery__container .ele-gallery-item.show{
				    	position:relative;
				    }
				    .elementor .elementor-element-'.$this->get_id().'  .elementor-gallery__container .ele-gallery-item.hide{
				    	position:absolute;
				    }';
		}
		if(($settings['gallery_type'] == 'single' || $settings['gallery_type'] == 'multiple') && ($settings['gallery_layout'] == 'grid' || $settings['gallery_layout'] == 'masonry') ){
			$inline_styles .= '.elementor .elementor-element-'.$this->get_id().' .elementor-gallery__container{
				    	width: 100%;
				    	display: inline-grid; 
				    	flex-wrap: wrap; 
				    	grid-template-columns: '.implode(' ', $columns_arr).';
		...
		$amp_elemetor_custom_css['amp-gallery'][$this->get_id()] = $inline_styles;
	}
	public function amp_elementor_gallery_scripts($data){
		$data['amp_component_scripts']['amp-lightbox-gallery'] = 'https://cdn.ampproject.org/v0/amp-lightbox-gallery-0.1.js';
		return $data;
	}
	public function amp_elementor_gallery_css(){
		echo '.e-gallery-image amp-img.agi img {object-fit: cover;}';
	}
	/**
	 *
	 */
	protected function render() {
		add_filter('amp_post_template_css', [$this,'amp_elementor_gallery_css'],11);
		$settings = $this->get_settings_for_display();
		$ideal_row_height = $settings['ideal_row_height'];
		$light_box = "";
		if ( 'file' === $settings['link_to'] ) {
			$light_box = 'lightbox';
			add_filter('amp_post_template_data', [$this, 'amp_elementor_gallery_scripts']);
		}
		...
		if ( $is_multiple ) {
			...
			<div <?php echo $this->get_render_attribute_string( 'titles-container' ); ?>>
				<?php if ( $settings['show_all_galleries'] ) { ?>
					<span href="#" data-gallery-index="all" class="elementor-item elementor-gallery-title" on="tap:AMP.setState({ multigal: -1 })" role="button" tabindex="55"><?php echo $settings['show_all_galleries_label']; ?></span>
				
				<?php } /*<amp-state id="multigal_<?php echo $index;?>"><script type="application/json"></script></amp-state>*/ ?>
				...
				<?php foreach ( $settings['galleries'] as $index => $gallery ) :
					...
					?>
					<span data-gallery-index="<?php echo $index; ?>" class="elementor-item elementor-gallery-title" on="tap:AMP.setState({ multigal: <?php echo $index;?> })"  role="button" tabindex="<?php echo $index;?>"><?php echo $gallery['gallery_title']; ?></span>
					<?php
				endforeach; ?>
			</div>
			<?php
		...
		<div <?php echo $this->get_render_attribute_string( 'gallery_container' ); ?>>
			...
			foreach ( $gallery_items as $id => $tags ) :
					...
					if($light_box == 'lightbox'){
						$this->add_render_attribute( 'gallery_item_image_' . $unique_index,
							[
								...
								'[class]' => "multigal==".implode( ',', $tags )." || multigal==-1? 'e-gallery-item elementor-gallery-item show':'e-gallery-item elementor-gallery-item hide'",
						...
						$this->add_render_attribute( 'gallery_item_' . $unique_index, [ 'data-e-gallery-tags' => implode( ',', $tags ),'[class]' => "multigal==".implode( ',', $tags )." || multigal==-1? 'e-gallery-item elementor-gallery-item show':'e-gallery-item elementor-gallery-item hide'",
						'class' => 'show' ] );
					}
				}

				if ( 'a' === $gallery_item_tag && $light_box != 'lightbox' ) {
					if ( 'file' === $settings['link_to'] ) {
				...
				<<?php echo $gallery_item_tag; ?> <?php echo $this->get_render_attribute_string( 'gallery_item_' . $unique_index ); ?>>
					...
					if($settings['aspect_ratio'] == '21:9'){
						$height = '60px';
					} ?>
					<div <?php echo $this->get_render_attribute_string( 'gallery_item_image_' . $unique_index ); ?> style="position: relative; width: 100%; height: <?php echo $height?>;">
						<amp-img class="agi" <?php echo $light_box_attr;?>alt="<?php echo $image_data['alt'];?>"  src="<?php echo $image_data['src'];?>"  width="<?php echo $image_data['width'];?>"  height="<?php echo $image_data['height'];?>"  layout="fill"></amp-img>
					</div>
					<?php 
					if($light_box != "lightbox"){
		...
		<span class="ele-gallery-item hide"></span>
	<?php }
	}
}
