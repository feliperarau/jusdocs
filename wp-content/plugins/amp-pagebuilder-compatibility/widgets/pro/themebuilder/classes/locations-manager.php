...
namespace ElementorForAmp\Widgets\Pro\Themebuilder\Classes;

use Elementor\Core\Files\CSS\Post as Post_CSS;
use ElementorPro\Core\Utils;
use ElementorPro\Modules\ThemeBuilder\Documents\Theme_Document;
use ElementorPro\Modules\ThemeBuilder\Module as Module;
...
class Locations_Manager {
 ...
 // 11 = after WooCommerce.
       add_filter( 'amp_post_template_file', [ $this,'template_include'],11,3);
...
/*		if ( ! Module::is_preview() ) {
	...
	public function template_include( $template,$type, $post  ) {

		//print_r(Module::instance()->get_conditions_manager());die;
		//$tedt  = new \ElementorPro\Modules\ThemeBuilder\Classes\Conditions_Cache();
...
//print_r(Module::instance()->get_conditions_manager()->get_documents_for_location( $location));die;
		...
		if ( ! empty( $page_template ) ) {
			...
			if ( $template_path ) {
               ...
               if($page_template == 'elementor_header_footer' && 'single' === $type){
               	
               	 $template = AMP_PAGEBUILDER_COMPATIBILITY_PLUGIN_DIR.'widgets/pro/themebuilder/templates/header-footer.php';
               	  $template = apply_filters("amp_pc_elementor_theme_builder_temeplate",$template);
			   }
          /*else{
			   	$template = $template_path;
			   }*/
