...
abstract class Skin_Base extends Elementor_Skin_Base {
	...
	public function render() {
		add_action('amp_post_template_css',[$this,'elementor_skinbase_stylesheet']);
	...
	protected function render_thumbnail() {
		...
		$thumbnail_html = preg_replace('/<img(.*?)width="(.*?)"(.*?)height="(.*?)"(.*?)src="(.*?)"(.*?)>/s', '$2-^--$4-^--$6', $thumbnail_html);

         $thumbnail_html_array = explode('-^--', $thumbnail_html);
		...
		<a class="elementor-post__thumbnail__link" href="<?php echo $this->current_permalink; ?>">
			<div class="elementor-post__thumbnail_amp">
		<amp-img src="<?php echo $thumbnail_html_array[2]; ?>" layout="responsive" width="<?php echo $thumbnail_html_array[0]; ?>" height="<?php echo $thumbnail_html_array[1]; ?>" ></amp-img>
	    </div>
		</a>
		<?php
	}
...
<?php }
		...
		<<?php echo $tag; ?> class="elementor-post__title">
			<a href="<?php echo $this->current_permalink; ?>" <?php 
			if(!isset($optional_attributes_html)){
   			 $optional_attributes_html = '';
    		}
	...
	function elementor_skinbase_stylesheet(){
	echo ".el-p-c{grid-column-gap:20px;grid-row-gap:20px}.ele-pst_title{padding:0 30px}.post_cat{padding: 0 30px;}";
	}
}
