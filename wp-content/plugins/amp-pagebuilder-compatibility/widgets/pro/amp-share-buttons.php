...
class Amp_Share_Buttons extends Base_Widget {
			...
			foreach ( $settings['share_buttons'] as $button ) {
				...
				<div class="elementor-grid-item">
					<div class="<?php echo esc_attr( $button_classes . $social_network_class ); ?>">
						<?php 
						$href = "";
						if(isset(self::$networks[$network_name]['url'])){
						 $network_url = self::$networks[$network_name]['url']; 
						 $href = str_replace(array('{url}', '{title}', '{text}','{image}'), array(get_the_permalink(), get_the_title() , get_the_excerpt(),
						 	get_the_post_thumbnail_url()), $network_url);
						}
						 ?>
						 
					<a target="_blank" href="<?php echo esc_url_raw($href);?>">
