...
class amp_ue_Image_Gallery extends Common_Widget {
	...
	add_action( 'amp_post_template_css',array( $this, 'custom_css_uea_gallery' ) );
...
function custom_css_uea_gallery(){?>
...
function custom_css_uea_carousel_gallery(){?>
	...
	protected function render_gallery_image( $images ) {
		...
		if ( 'carousel' == $settings['gallery_style'] ){
			...
			if( $settings['autoplay'] == 'yes'){
	             $autoSlider = 'autoplay delay="'.$settings['autoplay_speed'].'"';
	        }
            
         $output .= '<amp-carousel class="amp-uea_gallery" width="150" height="150" layout="responsive" type="slides" '.$autoSlider.'>';
        }
		foreach ( $new_gallery as $index => $item ) {
			...
			if ( 'justified' === $settings['gallery_style'] ) {
				...
				$output         .= '<div class="uael-grid-item ' . implode( ' ', $image_cat ) . '">';
					$output     .= '<div class="uael-grid-item-content justify-galley">';
			...
			// amp carousel markup.
			elseif( 'carousel' == $settings['gallery_style']){
				add_action( 'amp_post_template_css',array( $this, 'custom_css_uea_carousel_gallery' ) );
                 $image_link = wp_get_attachment_image_src( $item['id'], 'small' );
                 $image_link = $image_link[0];
				 $output .='<amp-img src="'.$image_link.'" width="150" height="150" layout="responsive" alt="a sample image"></amp-img>';
			}else{
