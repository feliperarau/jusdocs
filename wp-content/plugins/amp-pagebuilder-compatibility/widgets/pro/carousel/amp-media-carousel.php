...
class Amp_Media_Carousel extends Base {
	...
	protected function print_slide( array $slide, array $settings, $element_key ) {
		if ( 'video' === $slide['type'] && $slide['video']['url'] ) {	
				$get_car_video_type  = explode('/', $slide['video']['url']);
				echo '<div class="ele-carousel-image">
				    <amp-img on="tap:video-'.$slide['image']['id'].'" role="button" tabindex="0" class="block rounded" width="346" height="200" noloading="" src="'.$slide['image']['url'].'" layout="responsive"></amp-img>
				    <div on="tap:video-'.$slide['image']['id'].'" role="button" tabindex="0" class="ele-custom-embed-play">
				    <i class="eicon-play" aria-hidden="true"></i>
				    <span class="ele-screen-only">Play</span>
				      </div>
				    </div>';
				echo '<amp-lightbox id="video-'.$slide['image']['id'].'" layout="nodisplay">
				      <div class="lightbox dialog-close-button dialog-lightbox-close-button" on="tap:video-'.$slide['image']['id'].'.close" role="button" tabindex="0"><i class="eicon-close"></i></div>
				          <div class="modal-body">
				         ';
				if (stripos(json_encode($get_car_video_type),'vimeo') !== false) {
					echo '<amp-vimeo autoplay data-videoid="'.end($get_car_video_type).'" layout="fixed-height" height="260"></amp-vimeo>';
				}
				elseif (strpos(json_encode($get_car_video_type), 'youtu')) {
					echo '<amp-youtube height="500" layout="fixed-height" data-videoid="'.end($get_car_video_type).'"></amp-youtube>';
				}
				elseif (strpos(json_encode($get_car_video_type), 'vine')) {
					echo '<amp-vine height="260" layout="fixed-height" data-vineid="'.end($get_car_video_type).'"></amp-vine>';
				}
				elseif (strpos(json_encode($get_car_video_type), 'twitter')) {
					echo '<amp-twitter height="260" layout="fixed-height" data-tweetid="'.end($get_car_video_type).'"></amp-twitter>';
				}
				elseif (strpos(json_encode($get_car_video_type), 'instagram')) {
					echo '<amp-instagram data-shortcode="'.end($get_car_video_type).'" height="1" layout="fixed-height"></amp-instagram>';
				}
				elseif (strpos(json_encode($get_car_video_type), 'facebook')) {
					echo '<amp-facebook class="amp-video" height="260" layout="fixed-height" data-embed-as="video" data-href="'.$slide['video']['url'].'"></amp-facebook>';
				}
				else {
				echo '<amp-iframe height="260" layout="fixed-height" sandbox="allow-scripts allow-same-origin allow-popups allow-presentation" allowfullscreen frameborder="0" src="'.$slide['video']['url'].'"><amp-img layout="fill" src="'.$slide['image']['url'].'" width="400" height="300" placeholder></amp-img> </amp-iframe>';
				}
				echo '</div>
				    </amp-lightbox>';
				return;
	...
	protected function print_slide_image( array $slide, $element_key, array $settings ) {
		$image_width = (isset($settings['width']['size']) && !empty($settings['width']['size']))? $settings['width']['size'].'px': '100%';
		$image_height = (isset($settings['height']['size']) && !empty($settings['height']['size']))? $settings['height']['size'].'px': 'auto';
		?>
		<div <?php echo $this->get_render_attribute_string( $element_key . '-image' ); ?>>
			<?php if ( 'image' === $slide['type'] && !empty($slide['image']['url']) ) : ?>
				
					<amp-img src="<?php echo $slide['image']['url'];?>" width="400" height="300" alt="a sample image" layout="responsive" style="width:<?php echo $image_width;?>;height: <?php echo $image_height;?>;"></amp-img>
