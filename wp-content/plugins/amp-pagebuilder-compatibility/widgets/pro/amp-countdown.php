...
class Amp_Countdown extends Base_Widget {
	...
	private function get_strftime( $instance, $due_date ) {
		$string = '<amp-date-countdown timestamp-seconds="'.$due_date.'" layout="fixed-height"
  					height="140"><template type="amp-mustache">';
		if ( $instance['show_days'] ) {
			$string .= $this->render_countdown_item( $instance, 'label_days', 'elementor-countdown-days', '{{d}}' );
		}
		if ( $instance['show_hours'] ) {
			$string .= $this->render_countdown_item( $instance, 'label_hours', 'elementor-countdown-hours', '{{h}}' );
		}
		if ( $instance['show_minutes'] ) {
			$string .= $this->render_countdown_item( $instance, 'label_minutes', 'elementor-countdown-minutes', '{{m}}' );
		}
		if ( $instance['show_seconds'] ) {
			$string .= $this->render_countdown_item( $instance, 'label_seconds', 'elementor-countdown-seconds', '{{s}}' );
		}
		$string .= '</template></amp-date-countdown>';
		return $string;
	}
	...
	private function render_countdown_item( $instance, $label, $part_class, $data ) {
		$string = '<div class="elementor-countdown-item"><span class="elementor-countdown-digits ' . $part_class . '">'.$data.'</span>';
	...
	protected function render() {
		add_filter('amp_post_template_css', [$this, 'amp_countdowm_styles']);
		...
		$string = $this->get_strftime( $instance, $due_date );
	...
	public function amp_countdowm_styles(){
		echo '.ew-countdown .elementor-countdown-item {
			    display: inline-block;
			}
			@media(max-width:382px){
			.ew-countdown amp-date-countdown .elementor-countdown-digits {
			    font-size: 60px;
			}
			.ew-countdown amp-date-countdown .elementor-countdown-label {
			    font-size: 17px;
			}
			}';
	}
