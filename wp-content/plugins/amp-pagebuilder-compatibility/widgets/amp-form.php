...
class Amp_Form extends Form_Base {
	public $addRecaptcha = false;
	...
	public function amp_form_css(){
		echo '.elementor-form > .loading-message {
		    display: none;
		    margin-top: var(--space-1);
		  }

		 .elementor-form.amp-form-submitting > .loading-message {
		    display: block;
		  }
		  form .ebtn[type="submit"]{
	...
	protected function render() {
		add_action("amp_post_template_data", array($this, 'amp_mustach_script'));
		add_action("amp_post_template_css", array($this, 'amp_form_css'));
      ...
      $actionXhrUrl = preg_replace('#^http?:#', '', $submit_url);
      ?>
<span class="elementor-form amp-form-submitting"></span>
			<?php
			$schema_cls_appnd = "";
			if( function_exists ('saswp_schema_markup_output') ) {
			$schema_cls_appnd = 'saswp-review-submission-form';
			}
			?>
		<amp-script src="<?php echo str_replace('http:','https:',AMP_PAGEBUILDER_COMPATIBILITY_PLUGIN_DIR_URI);?>assets/amp-form-el.js" sandbox="allow-forms" class="sample">
		<form class="elementor-form ampforwp-form-allow <?php echo $schema_cls_appnd; ?>" method="post" <?php echo $this->get_render_attribute_string( 'form' ); ?> 
		action-xhr="<?php echo $actionXhrUrl;?>" target="_top">

<!-- $cf7Content = preg_replace( '#action="([^"]+)"#ius', 'action-xhr="' . $actionXhrUrl . '" target="_top"', $cf7Content ); -->
<?php if(class_exists('\ElementorPro\Classes\Utils')){ ?>
		<input type="hidden" name="post_id" value="<?php echo \ElementorPro\Classes\Utils::get_current_post_id(); ?>"/>
<?php }else{ ?>
...
<?php }?>
			...
			<div <?php echo $this->get_render_attribute_string( 'wrapper' ); ?>>
				...
				<div <?php echo $this->get_render_attribute_string( 'field-group' . $item_index ); ?>>
					...
					switch ( $item['field_type'] ) :
						case 'step':
								echo '<div hidden class="aelform_btn_">'.$item['next_button'].'</div>';
						break;
						case 'html':
						...
						case 'recaptcha':
							...
							$this->addRecaptcha = true;
							$sitekey = $redux_builder_amp['elem-recaptcha-sitekey'];
							$secretkey = $redux_builder_amp['elem-recaptcha-secretkey'];
							echo '<amp-recaptcha-input  layout="nodisplay" name="g-recaptcha-response" data-sitekey="'.$sitekey.'" data-action="g_recaptcha_response"></amp-recaptcha-input>';
							/*echo '<amp-recaptcha-input layout="nodisplay" name="recaptcha_token" data-sitekey="'. $site_key.'"  data-action="recaptcha_example">
              </amp-recaptcha-input>';*/
							/*$recaptcha = new \ElementorPro\Modules\Forms\Classes\Recaptcha_Handler();
							ob_start();
							$recaptcha->render_field($item, $item_index, $this);
							$recaptchCnt = ob_get_contents();
							ob_clean();
							
							$recaptchCnt = preg_replace("/<div\sclass=\"ele-g-recaptcha\"(.*?)><\/div>/", 
								'<amp-recaptcha-input layout="nodisplay" name="elementor-g-recaptcha" $1 data-action="elementor-g-recaptcha">
              </amp-recaptcha-input>', $recaptchCnt);
							echo $recaptchCnt;*/
						break;
						default:
							$field_type = $item['field_type'];
				...
				<div <?php echo $this->get_render_attribute_string( 'submit-group' ); ?>>
					<button id="amp-submit-btn" type="submit" <?php echo $this->get_render_attribute_string( 'button' ); ?>>
			...
			<input type="hidden" name="action" value="<?php echo 'elementor_amp_cform_submission'; ?>"/>
	...
	function amp_mustach_script($data){
		if(\ElementorPro\Modules\Forms\Classes\Recaptcha_Handler::is_enabled() && $this->addRecaptcha==true){
