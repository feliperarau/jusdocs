<?php
namespace ElementorForAmp\Widgets\EssentialAddon;
...
class Amp_ea_Post_Carousel extends Widget_Base {
    ...
    protected function render() {
        ...
        <div <?php echo $this->get_render_attribute_string( 'eael-post-carousel-container' ); ?>>
            <div <?php echo $this->get_render_attribute_string( 'eael-post-carousel-wrap' ); ?>>
                <div class="swpr-wrpr">
                    <amp-carousel class="ea-post-crsl" height="530" layout="fixed-height" type="carousel" controls>
                    ...
                    if ( $query->have_posts() ) {
                        while ( $query->have_posts() ) {
                            ...
                            if ( $settings['eael_post_carousel_preset_style'] === 'two' ) {
                                ...
                                self::amp_ea_render_template__thumnail( $settings, 'two' );
                                if ( $settings['eael_show_title'] || $settings['eael_show_meta'] || $settings['eael_show_excerpt'] ):
                                    ...
                                    echo '</div>';
                                    self::amp_ea_render_template__excerpt( $settings );
                                ...
                                <div class="eael-grid-post-holder">
                                    <div class="eael-grid-post-holder-inner">';
                                self::amp_ea_render_template__thumnail( $settings, 'three' );
                                if ( $settings['eael_show_title'] || $settings['eael_show_meta'] || $settings['eael_show_excerpt'] ):
                                    echo '<div class="eael-entry-wrapper">';
                                ...
                                <div class="eael-grid-post-holder">
                                    <div class="eael-grid-post-holder-inner">';
                                self::amp_ea_render_template__thumnail( $settings );
                                if ( $settings['eael_show_title'] || $settings['eael_show_meta'] || $settings['eael_show_excerpt'] ):
                                    echo '<div class="eael-entry-wrapper">';
    ...
    private static function amp_ea_render_template__thumnail( $settings, $style = null ) {
        if (  ( $settings['eael_show_image'] == '0' || $settings['eael_show_image'] == 'yes' ) && has_post_thumbnail() ) {
            echo '<div class="eael-entry-media eael-entry-medianone"><a href="' . ampforwp_url_controller(get_the_permalink()) . '">';
            echo '<div class="eael-entry-thbnl">
                    <img src="' . esc_url( the_post_thumbnail('medium') ) . '" alt="' . esc_attr( get_post_meta( get_post_thumbnail_id(), '_wp_attachment_image_alt', true ) ) . '">
                </div>';
            echo '</a></div>';
    ...
    private static function amp_ea_render_template__excerpt( $settings ) {
        ...
        if ( class_exists( 'WooCommerce' ) && $settings['post_type'] == 'product' ) {
            ...
            echo '<div class="eael-post-elements-readmore-wrap"><a href="' . ampforwp_url_controller(get_the_permalink()) . '" class="eael-post-elements-readmore-btn">' . esc_attr( $settings['read_more_button_text'] ) . '</a></div>';
        }
        echo '</div>
            </div>';
