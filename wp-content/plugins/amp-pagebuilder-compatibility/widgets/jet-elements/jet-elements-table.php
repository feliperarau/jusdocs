<?php
namespace ElementorForAmp\Widgets\JetElements;
...
use Elementor\Jet_Elements_Base;
use Elementor\Group_Control_Image_Size;

if ( ! defined( 'ABSPATH' ) ) exit; // Exit if accessed directly

class Amp_Jet_Elements_Table extends Jet_Elements_Base {
	...
	protected function render() {
		add_action('amp_post_template_css',[$this,'amp_jet_elements_table_styles']);
	 ...
	 * @return string
	 */
	public function amp_jet_elements_table_styles(){
		$settings = $this->get_settings_for_display();
		$table_head_cell_bg_color = '';
		$inline_styles = '';
		if(isset($settings['table_head_cell_bg_color']) && !empty($settings['table_head_cell_bg_color'])){
			$table_head_cell_bg_color = $settings['table_head_cell_bg_color'];
			$inline_styles .= '.elementor-element.elementor-element-'.$this->get_id().' .jet-table__head-cell{background-color:'.$table_head_cell_bg_color.';}';
		}
		$table_head_cell_align = '';
		if(isset($settings['table_head_cell_align']) && !empty($settings['table_head_cell_align'])){
			$table_head_cell_align = $settings['table_head_cell_align'];
			$inline_styles .= '.elementor-element.elementor-element-'.$this->get_id().' .jet-table__head-cell .jet-table__cell-content{text-align:'.$table_head_cell_align.';margin-left: auto;margin-right: auto;}';
		}
		$table_width = '';
		$table_align = '';
		$margin_left = 'auto';
		$margin_right = 'auto';
		if(isset($settings['table_width']['size']) && !empty($settings['table_width']['size'])){
			if(isset($settings['table_align']) && !empty($settings['table_align'])){
				if($settings['table_align'] == 'left'){
					$margin_left = '0';
					$margin_right = 'auto';
				}
				if($settings['table_align'] == 'right'){
					$margin_left = 'auto';
					$margin_right = '0';
				}
			}
			$table_width = $settings['table_width']['size'].''.$settings['table_width']['unit'];
			$inline_styles .= '.elementor-element.elementor-element-'.$this->get_id().' .jet-table-wrapper{width:'.$table_width.';margin-left: '.$margin_left.';margin-right: '.$margin_right.';}';
		}
		$table_body_cell_align = '';
		if(isset($settings['table_body_cell_align']) && !empty($settings['table_head_cell_align'])){
			$table_head_cell_align = $settings['table_head_cell_align'];
			$inline_styles .= '.elementor-element.elementor-element-'.$this->get_id().' .jet-table__body-cell .jet-table__cell-content{text-align:'.$table_body_cell_align.';margin-left: auto;margin-right: auto;}';
		}
