...
class AMP_Jet_Elements_Animated_Box extends Jet_Elements_Base {
	...
	public function __get_global_template_animated_box( $name = null ) {

		$template = call_user_func( array( $this, sprintf( '__get_%s_template', $this->__context ) ) );

		if ( ! $template ) {
			$template = AMP_PAGEBUILDER_COMPATIBILITY_PLUGIN_DIR . 'widgets/jet-elements/'.$this->get_name().'/global/' . $name . '.php';
		}
		return $template;
	}

	protected function render() {
        add_filter('amp_post_template_css', [$this,'jet_elements_animated_box_custom_css'],999);
		$this->__context = 'render';

		$this->_open_wrap();
		// include template files
		include $this->__get_global_template_animated_box( 'index' );
		$this->_close_wrap();
	}
	...
	function jet_elements_animated_box_custom_css(){
	    $settings = $this->get_settings();
		if($settings['switch_event_type'] == "hover" && $settings['animation_effect'] == "jet-box-effect-8"){
			echo ".jet-animated-box__front:hover {transform: rotateX(180deg);}.jet-animated-box__back:hover{transform: rotateX(0deg);}.ew-jet-animated-box .jet-animated-box__back {border-radius: 10px 10px 10px 10px;}";
		}
	}
