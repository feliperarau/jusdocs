...
class Amp_Jet_Elements_Animated_Text extends Jet_Elements_Base {
	...
	public function str_to_spanned_html( $base_string, $split_type = 'symbol' ) {
		...
		foreach ( $symbols_array as $symbol ) {
			...
			$spanned_array[] = sprintf( '%s', $symbol );
		}
	...
	protected function render() {
		...
		add_filter('amp_post_template_css', [$this,'amp_animated_text_custom_css'],999);
		...
		echo '<'.$tag.' class="jet-animated-text">';
		$this->_glob_inc_if( 'before-text', array( 'before_text_content' ) );
		echo '<div class="jet-animated-text__animated-text"><amp-carousel class="jet-animtext-crsl" type="slides" width="400" height="115" loop autoplay delay="7000" role="region" aria-label="Animated text">';
		$this->amp_animated_text_looped_template( 'animated-text', 'animated_text_list' );
		echo '</amp-carousel><span class="amp-carousel-button" style="display:none"><span></div>';
		$this->_glob_inc_if( 'after-text', array( 'after_text_content' ) );
		echo '</'.$tag.'>';
	}

	public function amp_animated_text_looped_template( $name = null, $setting = null, $callback = null ) {
		$templates = array(
	...
	function amp_animated_text_custom_css(){
		echo '.jet-animtext-crsl .amp-carousel-button{
    			display: none;
			}
			...
			.jet-animtext-crsl .jet-animated-text__animated-text-item {
			    visibility: visible;
			    line-height: 40px;
			}';
	}
