<?php
namespace ElementorForAmp\Widgets\JetBlog;
...
use Elementor\Jet_Blog_Base;
use Elementor\Utils;

if ( ! defined( 'ABSPATH' ) ) exit; // Exit if accessed directly

class Amp_Jet_Blog_Smart_Tiles extends Jet_Blog_Base {
	...
	public function _get_global_template( $name = null ) {
		...
		if ( ! $template ) {
			$template = AMP_PAGEBUILDER_COMPATIBILITY_PLUGIN_DIR . 'widgets/jet-blog/'.$this->get_name().'/global/' . $name . '.php';
		}
		return $template;
	}
	public function amp_pbc_jet_blog_smart_tiles_styles(){
		echo '.elementor .jet-smart-tiles__box amp-img{height:100%;width:100%;position: absolute;}
		.jet-smart-tiles .jet-smart-tiles__box{padding:0px;}';
	}
	protected function render() {
		add_action('amp_post_template_css',array($this,'amp_pbc_jet_blog_smart_tiles_styles'));
