...
class AMP_jxNav_mn extends Base_Widget {
	...
	protected function render() {
		...
		$args = [
			...
			'container' => '',
			'echo' => false,
			'walker' => new Classes\Walker_Nav_Menu(),
		];
	...
	public function render_center_logo( $output, $menu_items, $current_index ) {
		...
		if ( ! empty( $retina_image_src ) ) {
			$this->add_render_attribute( 'logo_image', 'data-jupiterx-rjs', $retina_image_src, true );
		}
	...
	*/
	public function render_side_logo() {
		...
		if ( ! empty( $retina_image_src ) ) {
			$this->add_render_attribute( 'side_logo_image', 'data-jupiterx-rjs', $retina_image_src, true );
		}
