...
namespace ElementorForAmp\Widgets\JetBlocks;
...
use Elementor\Jet_Blocks_Base;
use Elementor\Utils;

if ( ! defined( 'ABSPATH' ) ) exit; // Exit if accessed directly

class Amp_Jet_Blocks_Search extends Jet_Blocks_Base {
	...
	protected function render() {
		...
		add_action('amp_post_template_css',array($this,'amp_pbc_jet_block_search_styles'));
		...
		if ( 'true' === $settings['show_search_in_popup'] ) {
			...
			?>
			<div class="jet-psrch" id="jet-search">
	            <form role="search" method="get" class="jet-search__form" action="<?php echo esc_url( home_url( '/' ) ); ?>" target="_top">
	            	<input type="text" placeholder="AMP" value="<?php echo $amp_query_val ?>" name="<?php echo $amp_query_variable ?>" class="hidden"/>
				...
				<a title="close" class="jps-x" href="#">
					<?php $this->__icon( 'search_close_icon', '<button type="button" class="jet-search__popup-close"><span class="jet-search__popup-close-icon jet-blocks-icon">%s</span></button>' ); ?>
				</a>
			    </div>
			...
			<div class="jet-search__popup-trigger-container">
				<a title="search" class="jet-psrc" href="#jet-search"><?php
	...
	public function amp_pbc_jet_block_search_styles(){
		echo '.jet-search__field, .jet-search__submit {padding: 8px 12px;
