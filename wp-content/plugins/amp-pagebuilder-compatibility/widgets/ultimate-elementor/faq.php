...
namespace ElementorForAmp\Widgets\UltimateElementor;
 ...
 * Class FAQ.
 */
class amp_uae_FAQ extends Common_Widget {
	...
	function faq_uea_custom_css(){
		echo '.uael-accordion-title.fa-angle-right:before {
			    margin-right: 10px;
			}
	...
	protected function render() {
		add_action( 'amp_post_template_css',array( $this, 'faq_uea_custom_css' ) );
				...
				<div <?php echo wp_kses_post( $this->get_render_attribute_string( 'uael-faq-container' ) ); ?> >
					...
					foreach ( $settings['tabs'] as $key ) {
						...
						if ( ! ( '' === $key['question'] || '' === $key['answer'] ) ) {
							...
							$heading_size_tag = $heading_size_tag == 'span' ? 'h5' : $heading_size_tag;
							?>
							<div <?php echo wp_kses_post( $this->get_render_attribute_string( 'uael_faq_accordion_' . $key['_id'] ) ); ?> role="tablist">
								...
								<section class= "uael-accordion-faq" aria-expanded="false" role="tab">
									<<?php echo esc_html( $heading_size_tag ); ?> class="uael-accordion-title uael-question-<?php echo esc_attr( $key['_id'] ); ?> uael-question-span fas fa-angle-right" tabindex="0" id="uael-faq-<?php echo esc_attr( $count ); ?>"><?php echo wp_kses_post( $key['question'] ); ?></<?php echo esc_html( $heading_size_tag ); ?>>
