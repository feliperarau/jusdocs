...
function oxy_gallery_amp_pb_compatibility_for_oxygen($atts, $content = null, $name = null ) { 
  global $options;
   $options = array(
   ...
   //print_r($options); die;
   $oxy_gallery_amp = new Oxygen_VSB_Gallery($options);
    ...
    function get_ids_from_woocommerce() {
        ...
        $product_id = ($oxy_gallery_amp->param_array['woo_product_id']!="") ? $oxy_gallery_amp->param_array['woo_product_id'] : get_the_ID();
     ...
     * @author Louis
     */
    //add_action('amp_post_template_css', 'generate_css_amp');

    function generate_css_amp( ) {
      //echo "string";die;
        ...
        if ($oxy_gallery_amp->param_array['layout'] == 'flex') {          

            $width = number_format((100 / $oxy_gallery_amp->param_array['images_per_row']), 3)."%";

            if ($oxy_gallery_amp->param_array['space_around_image'] != '') {
                $width = "calc(".$width." - ".$oxy_gallery_amp->param_array['space_around_image']."px - ".$oxy_gallery_amp->param_array['space_around_image']."px)";
            }

            $aspect_ratio = explode(":", $oxy_gallery_amp->param_array['image_aspect_ratio']);
            ...
            <?php echo $selector; ?>.oxy-gallery-flex .oxy-gallery-item {
                ...
                margin: <?php echo $oxy_gallery_amp->param_array['space_around_image']; ?>px;
                min-width: <?php echo $oxy_gallery_amp->param_array['image_min_width']; ?>px;
                overflow: hidden;
                flex: auto;
            }
        ...
        <?php } 
        else if ($oxy_gallery_amp->param_array['layout'] == 'masonry') { ?>
                <?php echo $selector; ?>.oxy-gallery.oxy-gallery-masonry {
                    column-width: <?php echo $oxy_gallery_amp->param_array['image_min_width']; ?>px;
                    column-count: <?php echo $oxy_gallery_amp->param_array['images_per_row']; ?>;
                    column-gap: <?php echo $oxy_gallery_amp->param_array['space_around_image']; ?>px;
                }

                <?php echo $selector; ?>.oxy-gallery-masonry .oxy-gallery-item {
                    margin-bottom: <?php echo $oxy_gallery_amp->param_array['space_around_image']; ?>px;
                }
        <?php } ?>
            ...
            <?php echo $selector; ?>.oxy-gallery-captions .oxy-gallery-item .oxy-gallery-item-contents figcaption:not(:empty) {
                ...
                left: 0;
                right: 0;
                background-color: <?php echo $oxy_gallery_amp->param_array['caption_background_color']; ?>; /* caption background color */
                padding: 1em;
                color: <?php echo oxygen_vsb_get_global_color_value($oxy_gallery_amp->param_array['caption_color']); ?>;  /* caption text color */
                font-weight: bold;
                ...
                line-height: 1;
                /*pointer-events: none;*/
                transition: <?php echo $oxy_gallery_amp->param_array['transition_duration']; ?>s ease-in-out opacity;
                display: block;
            }

        <?php 

        if ($oxy_gallery_amp->param_array['gallery_captions_only_hover'] == 'yes') { ?>
        ...
        if (isset($oxy_gallery_amp->param_array['hide_captions_below']) && $oxy_gallery_amp->param_array['hide_captions_below'] != 'never') { ?>

            @media (max-width: <?php echo $media_queries_list[$oxy_gallery_amp->param_array['hide_captions_below']]['maxSize']; ?>) {
              <?php echo $selector; ?>.oxy-gallery-captions .oxy-gallery-item .oxy-gallery-item-contents figcaption:not(:empty) {
                display: none;
              }
            }
        ...
        <?php } ?>
            ...
            <?php echo $selector; ?>.oxy-gallery .oxy-gallery-item {
              opacity: <?php echo $oxy_gallery_amp->param_array['image_opacity']; ?>;
              transition: <?php echo $oxy_gallery_amp->param_array['transition_duration']; ?>s ease-in-out opacity;
            }

            <?php echo $selector; ?>.oxy-gallery .oxy-gallery-item:hover {
              opacity: <?php echo $oxy_gallery_amp->param_array['image_opacity_hover']; ?>;
            }
  ...
  if ( !(isset($atts['preview']) && $atts['preview'] == 'true') && ! $oxy_gallery_amp->validate_shortcode( $atts, $content, $name ) ) {
            return '';
        }

        $options = (!isset($atts['preview']) || $atts['preview']!='true') ? $oxy_gallery_amp->set_options( $atts ) : $atts;

        $oxy_gallery_amp->param_array = shortcode_atts(
            array(
                "layout"                => "flex", // flex | masonry
                "images_per_row"        => '4',
                "space_around_image"    => '10', //px only
                ...
                "transition_duration"   => '0.3',
                "gallery_thumbnail_size" => '',
            ), $options, $oxy_gallery_amp->options['tag'] );
        ...
        if ($oxy_gallery_amp->param_array['lightbox'] == 'yes' && $oxy_gallery_amp->param_array['link'] == 'yes') { 
            add_action( 'wp_footer', array( $oxy_gallery_amp, 'output_js' ) );
        }

        $defaultcss = "<style>".file_get_contents(ABSPATH . 'wp-content/plugins/oxygen/component-framework/components/classes/gallery/gallery.css')."</style>";
        //print_r($defaultcss); die;
        $optioncss = $oxy_gallery_amp->generate_css('#'.$options['selector']);

        if ($oxy_gallery_amp->param_array['gallery_source'] =='medialibrary') {
            $image_ids = $oxy_gallery_amp->param_array['image_ids'];
        } 
        else if ($oxy_gallery_amp->param_array['gallery_source'] =='acf') {
            $image_ids = $oxy_gallery_amp->get_ids_from_acf($oxy_gallery_amp->param_array['acf_field']);
        } 
        else if ($oxy_gallery_amp->param_array['gallery_source'] =='woocommerce') {
            $image_ids = $oxy_gallery_amp->get_ids_from_woocommerce();
        }

        $gallery_images = $oxy_gallery_amp->get_from_media_library($image_ids, $oxy_gallery_amp->param_array['gallery_thumbnail_size']);
        if (!isset($options['classes'])) {
            $options['classes'] = "";
        }
        
        if ($oxy_gallery_amp->param_array['gallery_captions'] == 'yes') {
            $options['classes'] .= " oxy-gallery-captions";
        }

        if ($oxy_gallery_amp->param_array['layout'] == 'masonry') {
        ...
        if (!isset($atts['preview']) || $atts['preview']!='true') : ?>
        <div id="<?php echo esc_attr($options['selector']); ?>" class="<?php echo esc_attr($options['classes']); ?>" <?php do_action("oxygen_vsb_component_attr", $options, $oxy_gallery_amp->options['tag']); ?>>
        <?php endif; 
        //print_r($optioncss); die;
        //echo esc_attr($options['selector']); die;
        ...
        if ($oxy_gallery_amp->param_array['layout'] == 'flex') { ?>
                ...
                <?php foreach ($gallery_images as $gallery_image) {
                    ...
                    if ($oxy_gallery_amp->param_array['link'] == 'yes') {
                        $linktag = 'a';
        ...
        // frontend 
        if ($oxy_gallery_amp->param_array['link'] == 'yes' && $oxy_gallery_amp->param_array['lightbox'] == 'yes' && (!isset($atts['preview']) || $atts['preview'] != 'true')) { ?>
            <script type="text/javascript">
        ...
        $outputContent = apply_filters('oxygen_vsb_after_component_render', $outputContent, $oxy_gallery_amp->options, $name);
     ...
     * @author Ilya K.
     */
    add_action("ct_toolbar_component_settings", "settings");
    function settings () { 
  ...
  add_action('amp_post_template_css', 'amp_pb_compat_oxy_gallery_css');
  function amp_pb_compat_oxy_gallery_css(){
    $inline_styles = file_get_contents(ABSPATH . 'wp-content/plugins/oxygen/component-framework/components/classes/gallery/gallery.css');    
    $inline_styles .= '.oxy-gallery-flex .oxy-gallery-item-contents img {
  width: 100%;
  height: 100%;
  opacity: 1;
}';
echo $inline_styles;
  ...
  add_action('amp_post_template_css', 'generate_css_amp_ver');
  function generate_css_amp_ver( ) {
      $CT_Component_amp = new CT_Component($options);
      $validate_shortcode_amp = $CT_Component_amp->validate_shortcode;
      global $media_queries_list,$options;
        if ( !(isset($atts['preview']) && $atts['preview'] == 'true') && ! $CT_Component_amp->validate_shortcode( $atts, $content, $name ) ) {
            return '';
        }
      $set_options_amp = $CT_Component_amp->set_options;
      $options = (!isset($atts['preview']) || $atts['preview']!='true') ? $CT_Component_amp->set_options( $atts ) : $atts;
        ...
        if ($oxy_gallery_amp->param_array['layout'] == 'flex') {
          ...
          global $post;
  $post_Id = get_the_ID();
  $selector = '#_gallery-'.$options['selector'].'-'.$post_Id.'';

    $width = number_format((100 / $oxy_gallery_amp->param_array['images_per_row']), 3)."%";

            if ($oxy_gallery_amp->param_array['space_around_image'] != '') {
          //echo "stringooiij";die;
                $width = "calc(".$width." - ".$oxy_gallery_amp->param_array['space_around_image']."px - ".$oxy_gallery_amp->param_array['space_around_image']."px)";
            }
