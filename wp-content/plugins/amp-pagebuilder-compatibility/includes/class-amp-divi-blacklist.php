...
if(defined('AMP__VENDOR__DIR__')){
	$baseblackSanitizer = AMP__VENDOR__DIR__;
}elseif( defined('AMP__DIR__') ){
	$baseblackSanitizer = AMP__DIR__;
}
$amp_blacklist_sanitizer =  realpath( $baseblackSanitizer . 'includes/sanitizers/class-amp-blacklist-sanitizer.php') ;

if ( file_exists($amp_blacklist_sanitizer) ) {
	include_once $amp_blacklist_sanitizer;
}

if ( class_exists( 'AMP_Blacklist_Sanitizer' ) ) {
	class AMPFORWP_DIVI_Blacklist extends AMP_Blacklist_Sanitizer {
		...
		private function strip_attributes_recursive( $node, $bad_attributes, $bad_protocols ) {
			...
			if($node->nodeName=='a' && $node->hasAttribute('href')){
		        ...
		        if ( $href && function_exists('ampforwp_findInternalUrl') ){
		...
		private function validate_a_node( $node ) {
			...
			/* Convert space into %20 and esc url so it can work with the correct 
				return false;				urls that have spaces */
			if ( strpos($href, ' ') ){
				$href = esc_url($href);
			}
