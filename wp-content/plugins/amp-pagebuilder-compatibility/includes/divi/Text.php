<?php
if(!class_exists('AMP_PC_ET_Builder_Module_Text')){
class AMP_PC_ET_Builder_Module_Text extends ET_Builder_Module {
	function init() {
	...
	function render( $attrs, $content = null, $render_slug='' ) {
		...
		if(isset($attrs['link_option_url']) && !empty($attrs['link_option_url'])){
			add_action('amp_post_template_css',array($this,'amp_divi_text_css'));
			if(isset($attrs['text_text_color'])){
				$content = $multi_view->render_element( array(
					'tag'     => 'a',
					'content' => '{{content}}',
					'attrs'   => array(
						'href' => $attrs['link_option_url'],
						'Style' => 'color:'.$attrs['text_text_color'].'',
					),
				) );
			}else{
				$content = $multi_view->render_element( array(
			...
			$content = '<div class="et_pb_text_inner">'.$content.'</div>';
		}else{
			$content = $multi_view->render_element( array(
				'tag'     => 'div',
				'content' => '{{content}}',
	...
	function amp_divi_text_css(){
		echo ".etx.et_clickable { z-index: 9; }";
	}
}

$amptextObj = new AMP_PC_ET_Builder_Module_Text();
remove_shortcode( 'et_pb_text' );
add_shortcode( 'et_pb_text', array($amptextObj, '_render'));
}