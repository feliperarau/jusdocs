<?php
if(!class_exists('AMP_PC_ET_Builder_Module_Contact_Form')){
class AMP_PC_ET_Builder_Module_Contact_Form extends ET_Builder_Module {
	function init() {
	...
	function get_form_map( $content_shortcode = '', $contact_form_number = 0, $hidden_form_fields = array() ) {
		...
		foreach( $contact_fields as $contact_field ) {
			...
			$map['fields'][] = array(
				'field_id'      => 'et_pb_contact_'.$field_id.'_'.$contact_form_number,
				...
				'field_label' 	=> self::$_->array_get( $contact_field_attrs, 'field_title', 'input' )
			);
		}
	...
	function render( $attrs, $content = null, $render_slug='' ) {
		...
		$hidden_form_fields      = ET_Builder_Module_Contact_Form::$_->array_get( $_POST, $hidden_form_fields_key, array() );
		...
		$form_map = $this->get_form_map( $content, $et_pb_contact_form_num, $hidden_form_fields );
		$contact_fields = json_encode($form_map['fields']);
		...
		if ( $et_contact_error ) {
			...
			$schema_cls_appnd = function_exists ('saswp_schema_markup_output') ? 'saswp-review-submission-form' : '';
			$form = sprintf( '
				<div class="et_pb_contact">
					<form class="et_pb_contact_form ampforwp-form-allow '.$schema_cls_appnd.' clearfix" method="post" action-xhr="%1$s">
						%8$s
						<input type="hidden" value="et_contact_proccess" name="et_pb_contactform_submit_%7$s">
						<input type="hidden" value="'.$email.' " name="to_email">
						<input type="hidden" value="'.$custom_message.' " name="custom_message">
						<input type="hidden" value="'.htmlspecialchars($contact_fields,ENT_QUOTES, 'UTF-8').'" name="contact_fields">
						<input type="hidden" value="'.$et_pb_contact_form_num.' " name="unique_id">
    ...
    if(preg_match('/<input\stype="text"(.*?)>/', $output) || preg_match('/<textarea name="et_pb_contact_message(.*?)"(.*?)>/', $output) ){
		$output = preg_replace('/<input\stype="text"(.*?)>/', '<input type="text"$1 required >', $output);

		$output = preg_replace('/<textarea name="et_pb_contact_message(.*?)"(.*?)>/', '<textarea name="et_pb_contact_message$1"$2 required >', $output);
	}
...
$contactFormObj = new AMP_PC_ET_Builder_Module_Contact_Form();
