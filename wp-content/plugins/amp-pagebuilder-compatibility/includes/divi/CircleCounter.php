...
class AMP_ET_Builder_Module_Circle_Counter extends ET_Builder_Module {
	...
	function render( $attrs, $content = null, $render_slug='' ) {
		add_action( 'amp_post_template_css',array( $this, 'custom_css_for_amp_cc' ) );
...
// my code start here.	
	...
	if(!empty($link_options_data)){
	$target = $link_options_data[0]['target'];
	$link = $link_options_data[0]['url'];
		$anchorlink = '<a href="'.$link.'" target="'.$target.'">';
		$anchorclose = '</a>';
	}
    //get percent.
	$showPercentSign ="";
	...
	$output = $anchorlink.'
		  <div class="circle-wrap">
		    <div class="circle">
		      ...
		      <div class="inside-circle">'.$number.$showPercentSign.'</div>
		    </div>
		  </div>
		</div>
		<h3 class="et_pb_module_header">'.$title .'</h3>'.$anchorclose;

		return $output;
	}


function custom_css_for_amp_cc(){?>
.amp-cc {
    height: 225px;
}
.circle-wrap {
...
<?php $rotateDegree = $this->props['number']*1.8;
$rotateDegree ='rotate('.$rotateDegree.'deg)';
?>
.circle-wrap .circle .mask.full,
.circle-wrap .circle .fill {
  transform: <?php echo $rotateDegree;?>
}
...
new AMP_ET_Builder_Module_Circle_Counter;
