<?php
if(!class_exists('AMP_PC_ET_Builder_Module_Filterable_Portfolio')){
class AMP_PC_ET_Builder_Module_Filterable_Portfolio extends ET_Builder_Module_Type_PostBased {
	function init() {
	...
	function amp_divi_inline_styles(){
		echo '.e_filterable_portfolio_grid .e_portfolio_item{display:block;}.e_portfolio_items_wrapper .e_portfolio_items{display: inline-flex; flex-wrap: wrap;}
		.et_pb_grid_item.hide-category-filter{display:none;}';
	}
	...
	function render( $attrs, $content = null, $render_slug='' ) {
		...
		if( $projects->post_count > 0 ) {
			while ( $projects->have_posts() ) {
				...
				?>
				<div id="post-<?php the_ID(); ?>" <?php post_class( $main_post_class." active" ); ?> [class]="showfilterd!='all' && showfilterd!='<?php echo 'project_category_' . urldecode( $category->slug ); ?>'? '<?php echo implode( ' ',get_post_class( $main_post_class) )." hide-category-filter"; ?>': '<?php echo implode( ' ',get_post_class( $main_post_class) ); ?>'">
				<?php
					$thumb = '';
		...
		if ( ! $posts = ob_get_clean() ) {
			...
			$category_filters .= sprintf( '<li class="et_pb_portfolio_filter et_pb_portfolio_filter_all"><a href="#" class="active" data-category-slug="all" on="tap:AMP.setState({showfilterd:\'all\'})">%1$s</a></li>',
				esc_html__( 'All', 'et_builder' )
			);
			foreach ( $terms as $term  ) {
				$category_filters .= sprintf( '<li class="et_pb_portfolio_filter"><a href="#" data-category-slug="%1$s" on="tap:AMP.setState({showfilterd:\'project_category_%1$s\'})">%2$s</a></li>',
		...
		$output = sprintf(
			'<amp-state id="showfilterdportfolio">
			  <script type="application/json">
				{
				  "showfilterd": "all"
				}
			  </script>
			</amp-state><span class="et_pb_grid_item hide-category-filter"></span><div%4$s class="%1$s" data-posts-number="%5$d"%8$s%11$s%12$s>
				%10$s
				%9$s
...
$accordianFilterableObj = new AMP_PC_ET_Builder_Module_Filterable_Portfolio();//et_pb_accordion_item
remove_shortcode( 'et_pb_filterable_portfolio' );
add_shortcode( 'et_pb_filterable_portfolio', array($accordianFilterableObj, '_render'));
}