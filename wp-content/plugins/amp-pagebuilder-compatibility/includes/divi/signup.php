<?php
if(!class_exists('AMP_ET_Builder_Module_Signup')){
		...
		switch ( $field ) {
			case 'name':
				...
				$html      = sprintf( '
					<p class="et_pb_newsletter_field%1$s">
						...
						<input id="et_pb_signup_firstname" class="input" type="text" placeholder="%3$s" name="et_firstname">
					</p>',
			...
			case 'last_name':
				...
				$html      = sprintf( '
					<p class="et_pb_newsletter_field%1$s">
						...
						<input id="et_pb_signup_lastname" class="input" type="text" placeholder="%3$s" name="et_lastname">
					</p>',
					$fullwidth ? ' et_pb_contact_field_last' : ' et_pb_contact_field_half',
					esc_html( $label ),
					esc_attr( $label )
			...
			case 'email':
				...
				$html      = sprintf( '
					<p class="et_pb_newsletter_field%1$s">
						...
						<input id="et_pb_signup_email" class="input" type="text" placeholder="%3$s" name="et_email">
					</p>',
					$fullwidth ? ' et_pb_contact_field_last' : ' et_pb_contact_field_half',
					esc_html( $label ),
					esc_attr( $label )
			...
			case 'submit_button':
				...
				$html = sprintf( '
					<p class="et_pb_newsletter_button_wrap">
						<button type="submit" class="et_pb_newsletter_button et_pb_button%1$s" %2$s data-icon="%3$s">
			...
			case 'hidden':
				...
				if ( 'feedburner' === $provider ) {
					...
					$html = sprintf( '
						<input type="hidden" value="%1$s" name="et_provider" />
						<input type="hidden" value="%2$s" name="et_list_id" />
						<input type="hidden" value="%3$s" name="et_account_name" />
						<input type="hidden" value="%4$s" name="et_ip_address" />',
						esc_attr( $provider ),
						esc_attr( $list ),
		...
		if ( $list_selected && 'feedburner' === $provider ) {
			...
			$form = sprintf( '
				<div class="et_pb_newsletter_form">
					<form action-xhr="%10$s" method="post"%9$s>
				...
				preg_replace("#http:#", '', admin_url('admin-ajax.php?action=amp_divi_optin_newsletter_submit'))
			);
		}
...
$SignupObj = new AMP_ET_Builder_Module_Signup();
remove_shortcode( 'et_pb_signup_custom_field' );
add_shortcode( 'et_pb_signup_custom_field', array($SignupObj, '_render'));
}