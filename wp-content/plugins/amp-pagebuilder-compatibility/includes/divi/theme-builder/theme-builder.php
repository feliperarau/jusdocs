...
require_once AMP_PAGEBUILDER_COMPATIBILITY_PLUGIN_DIR.'/includes/divi/theme-builder/ThemeBuilderRequest.php';
require_once AMP_PAGEBUILDER_COMPATIBILITY_PLUGIN_DIR.'/includes/divi/theme-builder/template-setting-validations.php';
 ...
 * @return void
 */
function amp_et_theme_builder_register_entities() {
...
add_action( 'init', 'amp_et_theme_builder_register_entities', 11 );
 ...
 * @return string[]
 */
function amp_et_theme_builder_get_layout_post_types() {
	return array(
		ET_THEME_BUILDER_HEADER_LAYOUT_POST_TYPE,
		ET_THEME_BUILDER_BODY_LAYOUT_POST_TYPE,
		ET_THEME_BUILDER_FOOTER_LAYOUT_POST_TYPE,
 ...
 * @return string
 */
function amp_et_theme_builder_get_valid_layout_post_type( $layout_type ) {
	$map = array(
 ...
 * @return boolean
 */
function amp_et_theme_builder_is_layout_post_type( $post_type ) {
	return in_array( $post_type, amp_et_theme_builder_get_layout_post_types(), true );
}
 ...
 * @return string[]
 */
function amp_et_theme_builder_get_post_content_modules() {
 ...
 * @return array
 */
function amp_et_theme_builder_filter_builder_default_post_types( $post_types ) {
	return array_merge( $post_types, amp_et_theme_builder_get_layout_post_types() );
}
add_filter( 'et_builder_default_post_types', 'amp_et_theme_builder_filter_builder_default_post_types' );
add_filter( 'et_library_builder_post_types', 'amp_et_theme_builder_filter_builder_default_post_types' );
 ...
 * @return array
 */
function amp_et_theme_builder_filter_builder_post_type_options_blocklist( $post_types ) {
	return array_merge(
		$post_types,
		amp_et_theme_builder_get_layout_post_types(),
		array( ET_THEME_BUILDER_TEMPLATE_POST_TYPE )
	);
}
add_filter( 'et_builder_post_type_options_blocklist', 'amp_et_theme_builder_filter_builder_default_post_types' );
 ...
 * @return bool
 */
function amp_et_theme_builder_filter_enable_builder_for_post_types( $enabled, $post_id ) {
	$post_type = get_post_type( $post_id );

	if ( amp_et_theme_builder_is_layout_post_type( $post_type ) ) {
		$enabled = true;
	}

	return $enabled;
}
add_filter( 'et_builder_fb_enabled_for_post', 'amp_et_theme_builder_filter_enable_builder_for_post_types', 10, 2 );
 ...
 * @return integer
 */
function amp_et_theme_builder_get_theme_builder_post_id( $live, $create = true ) {
 ...
 * @return integer[]
 */
function amp_et_theme_builder_get_theme_builder_template_ids( $live ) {
	$post_id      = amp_et_theme_builder_get_theme_builder_post_id( $live, false );
 ...
 * @return array
 */
function amp_et_theme_builder_get_theme_builder_templates( $live ) {
	return array_filter(
		array_map(
			'et_theme_builder_get_template',
			amp_et_theme_builder_get_theme_builder_template_ids( $live )
		)
	);
}
 ...
 * @return array
 */
function amp_et_theme_builder_get_template( $template_id ) {
 ...
 * @return void
 */
function amp_et_theme_builder_trash_draft_and_unused_posts() {
	$mark_meta_key = '_et_theme_builder_marked_as_unused';
	$live_id       = amp_et_theme_builder_get_theme_builder_post_id( true, false );
	$draft_id      = amp_et_theme_builder_get_theme_builder_post_id( false, false );
 ...
 * @return array
 */
function amp_et_theme_builder_get_template_settings_options_for_post_type( $post_type_name ) {
	...
	$group = array(
		...
		'settings' => array(
			array(
				...
				'priority' => 70,
				'validate' => 'amp_et_theme_builder_template_setting_validate_singular_post_type_all',
			),
		),
	);
 ...
 * @return array
 */
function amp_et_theme_builder_get_template_settings_options_for_archive_pages() {
 ...
 * @return array
 */
function amp_et_theme_builder_get_template_settings_options() {
 ...
 * @return array[]
 */
function amp_et_theme_builder_get_template_settings_options_for_preloading() {
	$templates   = array_unique(
		array_merge(
			amp_et_theme_builder_get_theme_builder_template_ids( true ),
			amp_et_theme_builder_get_theme_builder_template_ids( false )
		)
	);
	$setting_ids = array();
	...
	return amp_et_theme_builder_load_template_setting_options( array_unique( $setting_ids ) );
}
 ...
 * @return string[]
 */
function amp_et_theme_builder_load_template_setting_options( $setting_ids ) {
	$flat_parent_settings = amp_et_theme_builder_get_flat_template_settings_options();
	$groups               = array();
	...
	foreach ( $groups as $parent_id => $group ) {
		$settings = array_merge(
			$settings,
			amp_et_theme_builder_get_template_setting_child_options( $group['parent'], $group['settings'] )
		);
	}
 ...
 * @return array
 */
function amp_et_theme_builder_get_flat_template_settings_options() {
	$settings = amp_et_theme_builder_get_template_settings_options();
	$flat     = array();
...
function amp_et_theme_builder_get_template_setting_child_options( $parent, $include = array(), $search = '', $page = 1, $per_page = 30 ) {
 ...
 * @since 4.0
 *
 * @param Amp_ET_Theme_Builder_Request $request Request to check against. Defaults to the current one.
 ...
 */
function amp_et_theme_builder_get_template_layouts( $request = null, $cache = true, $load_from_cache = true ) {
	static $store = array();
	...
	if ( null === $request ) {
		...
		$request = Amp_ET_Theme_Builder_Request::from_current();
	}
	...
	$post_type = Amp_ET_Theme_Builder_Request::TYPE_SINGULAR === $request->get_type() ? $request->get_subtype() : '';
	$layouts   = array();
	...
	if ( amp_et_theme_builder_is_layout_post_type( $post_type ) ) {
		...
		$templates = amp_et_theme_builder_get_theme_builder_templates( true );
		$settings  = amp_et_theme_builder_get_flat_template_settings_options();
 ...
 * @return boolean
 */
function amp_et_theme_builder_overrides_layout( $layout ) {
 ...
 * @return boolean
 */
function amp_et_theme_builder_layout_has_post_content( $layout ) {
	if ( ! $layout['override'] ) {
	...
	if ( $layout['enabled'] ) {
		...
		$modules = amp_et_theme_builder_get_post_content_modules();
 ...
 * @return string
 */
function amp_et_theme_builder_get_failure_notification_modal( $template_name, $layout_enabled ) {
 ...
 * @return integer
 */
function amp_et_theme_builder_store_template( $theme_builder_id, $template, $allow_default ) {
	$_                   = et_();
 ...
 * @return array
 */
function amp_et_theme_builder_sanitize_template( $template ) {
	$_                   = et_();
 ...
 */
function amp_et_theme_builder_insert_layout( $options ) {
	$post_id = wp_insert_post(
		array_merge(
			array(
				'post_status' => 'publish',
 ...
 * @return string.
 */
function amp_et_theme_builder_cache_post_type( $post_type ) {
	if ( amp_et_theme_builder_is_layout_post_type( $post_type ) ) {
 ...
 * @return string
 */
function amp_et_theme_builder_decorate_page_resource_slug( $post_id, $resource_slug ) {
	...
	if ( amp_et_theme_builder_is_layout_post_type( $post_type ) ) {
		...
		$layout_types = amp_et_theme_builder_get_layout_post_types();
		$layouts      = amp_et_theme_builder_get_template_layouts();
 ...
 * @return void
 */
function amp_et_theme_builder_clear_wp_cache( $post_ids = 'all' ) {
 ...
 * @return void
 */
function amp_et_theme_builder_clear_wp_post_cache( $layout_id = '' ) {
	$layout_type = get_post_type( $layout_id );

	if ( ! amp_et_theme_builder_is_layout_post_type( $layout_type ) ) {
		return;
	}
	...
	if ( empty( $template_use_on ) ) {
		...
		if ( $is_template_default ) {
			amp_et_theme_builder_clear_wp_cache( 'all' );
		}
		return;
	}
	...
	amp_et_theme_builder_clear_wp_cache( $target_post_ids );
}

add_action( 'et_save_post', 'amp_et_theme_builder_clear_wp_post_cache' );
