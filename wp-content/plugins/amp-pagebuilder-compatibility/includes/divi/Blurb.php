<?php
if(!class_exists('AMP_PC_ET_Builder_Module_Blurb')){
class AMP_PC_ET_Builder_Module_Blurb extends ET_Builder_Module {
	function init() {
	...
	function amp_divi_inline_styles(){
		$inline_styles = "";
		$inline_styles .= '.et_pb_main_blurb_image .et_pb_image_wrap amp-img{max-width:100%;}';
		echo $inline_styles;
	}
	function render( $attrs, $content = null, $render_slug='' ) {
		...
		if ( 'off' === $use_icon ) {
			...
			preg_match('/<img(.*?)src=\"(.*?)\"(.*?)srcset=\"(.*?)\"(.*?)sizes=\"(.*?)\"(.*?)>/', $image, $matches);
			if(isset($matches[4])){
				$image = '<amp-img src="'.$image_url.'" width="'.$img_width.'" height="'.$img_height.'" srcset="'.$matches[4].'" layout="responsive"'.$matches[7].' ></amp-img>';
			}
		} else {
			...
			$custom_icon = strpos($image, 'agsdix-smc');
			...
			$image = str_replace('~|elegant-themes~', '', $image);
			$image = str_replace('|elegant-themes-icon~|', '', $image);
		}
		...
		if( function_exists('et_pb_background_layout_options') ){
	...
	if($this->props['link_option_url_new_window'] == 'on'){
		$link_option_url_new_window = 'target="_blank"';
	}
	if(isset($this->props['link_option_url']) && !empty($this->props['link_option_url'])){
		$url_start = '<a href="'.$this->props['link_option_url'].'" '.$link_option_url_new_window.'>';
		$url_end = '</a>';
	}
		...
		$output = sprintf(
			'<div%5$s class="a-blurb"%8$s>
				...
				</div>'.$url_end.' <!-- .et_pb_blurb_content -->
			</div> <!-- .et_pb_blurb -->',
			$content,
...
$blurbObj = new AMP_PC_ET_Builder_Module_Blurb();
