<?php
if(!class_exists('AMP_PC_ET_Builder_Module_Gallery')){
class AMP_PC_ET_Builder_Module_Gallery extends ET_Builder_Module {
	function init() {
	...
	static function get_gallery( $args = array(), $conditional_tags = array(), $current_page = array() ) {
		add_action( 'amp_post_template_css','amp_divi_gallery_custom_css',999);
		$attachments = array();
	...
	function render( $attrs, $content = null, $render_slug='' ) {
		...
		$amp_carousel = '';
		if ( 'on' == $fullwidth ) {
			$amp_carousel ='<amp-carousel height="300" width="300" type="slides" layout="responsive" loop>';
		}
...
function amp_divi_gallery_custom_css(){?>
.e_gallery_grid .e_gallery_item{display:block;}
.e_gallery_grid{width: 100%;max-width: 100%;}
@media (min-width: 981px){
	.e_gallery_grid .e_gallery_item {display: inline-block;vertical-align: top;margin-top: 5px;}
	.e_gallery_item.e_grid_item.e_bg_ll {width: 19%;}
}
@media (max-width:980px) and (min-width:768px) {
	.ec .e_grid_item {width: 25%;}
}
<?php }
$GalleryObj = new AMP_PC_ET_Builder_Module_Gallery();
