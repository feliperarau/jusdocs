<?php
if(!class_exists('AMP_ET_Builder_Module_Fullwidth_Header')){
class AMP_ET_Builder_Module_Fullwidth_Header extends ET_Builder_Module {
	function init() {
	...
	public function render( $attrs, $content, $render_slug ) {
		...
		if ( 'off' !== $header_scroll_down || '' !== $scroll_down_icon || '' !== $scroll_down_icon_tablet || '' !== $scroll_down_icon_phone ) {
			...
			$scroll_down_output .= sprintf(
				'<a href="#" class="scroll-down-container%4$s" on="tap:scrollElement.scrollTo(duration=200)" on="tap:scrollElement.scrollTo(duration=200)" role="button" tabindex="1">
...
$et_pb_fullwidth_header = new AMP_ET_Builder_Module_Fullwidth_Header();
remove_shortcode( 'et_pb_fullwidth_header' );
add_shortcode( 'et_pb_fullwidth_header', array($et_pb_fullwidth_header, '_render'));