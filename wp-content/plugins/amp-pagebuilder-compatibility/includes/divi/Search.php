<?php
if(!class_exists('AMP_PC_ET_Builder_Module_Search')){
class AMP_PC_ET_Builder_Module_Search extends ET_Builder_Module {
	function init() {
	...
	function render( $attrs, $content = null, $render_slug='' ) {
		...
		$output = sprintf(
			'<div%3$s class="%2$s"%12$s%13$s>
				%11$s
				%10$s
				<form role="search" method="get" class="et_pb_searchform" action="%1$s" target="_top">
					<div>
			...
			ampforwp_url_controller(esc_url( home_url( '/' ) )),
...
$searchObj = new AMP_PC_ET_Builder_Module_Search;
