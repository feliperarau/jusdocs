...
if(!class_exists('AMP_PC_ET_Builder_Module_Blog')){
class AMP_PC_ET_Builder_Module_Blog extends ET_Builder_Module_Type_PostBased {
	/**
		...
		$this->advanced_fields = array(
			'fonts'                 => array(
				...
				'read_more'  => array(
					...
					'css'             => array(
						'main'  => "{$this->main_css_element} a.more-link",
						'hover' => "{$this->main_css_element} a.more-link:hover",
					),
	...
	public function amp_divi_inline_styles(){
		...
		if ( 'off' === $fullwidth ){
			$inline_styles .= "@media only screen and (min-width: 981px) {.e_salvattore_content_amp{display: grid;grid-template-columns: 1fr 1fr 1fr;column-gap: 2rem;}}";
		}
		echo $inline_styles;
	}
	...
	function render( $attrs, $content = null, $render_slug='' ) {
		...
		if ( have_posts() ) {
			if ( 'off' === $fullwidth ) {
				...
				echo '<div class="et_pb_salvattore_content_amp" ' . et_core_intentionally_unescaped( $attribute, 'fixed_string' ) . '>';
			}
...
$BlogObj = new AMP_PC_ET_Builder_Module_Blog();
