<?php
if(!class_exists('AMP_PC_ET_Builder_Module_Video_Slider')){
class AMP_PC_ET_Builder_Module_Video_Slider extends ET_Builder_Module {
	function init() {
	...
	function render( $attrs, $content = null, $render_slug='' ) {
		$ovarlayOn = $this->props['show_image_overlay'];
		if($ovarlayOn == 'on'){
		 add_filter('ampforwp_the_content_last_filter',array($this,'ampforwp_purify_vdoslideroverlay'));
		}
		//add_action('amp_post_template_css',array($this,'tester_styles'),999);
		...
		$output = sprintf(
			'<div%3$s class="%4$s">
				...
				<div class="et_pb_slider et_pb_preload%1$s"%7$s>
					...
					<amp-carousel height="300" width="300" type="slides" layout="responsive" controls>
						%2$s
					</div>
	...
	function ampforwp_purify_vdoslideroverlay($completeContent){
        //$src = $this->props['src'];
		//$word = "vimeo.com";
		//if(strpos($src, $word) !== false){
		  ...
		  //$vimeoId = $vimeoId[3]; 
			...
			//$completeContent = preg_replace('/<div class="e_video_box"><amp-iframe src="(.*?)" width="(.*?)" height="(.*?)" (.*?)><\/amp-iframe>/', '<div class="e_video_box"><amp-vimeo data-videoid="'.$vimeoId.'" autoplay layout="responsive" width="500" height="281" id="myVideo"></amp-vimeo>', $completeContent);

			preg_match_all('/<div class="e_video_overlay acss(.*?)">/', $completeContent, $matches);
			$i=0;
			for($i=0; $i<count($matches[0]); $i++){
				$completeContent = preg_replace('/<div class="e_video_overlay acss(.*?)">/', '<div id="'.$matches[1][$i].$i.'" class="e_video_overlay acss'.$matches[1][$i].'">', $completeContent, 1 );
				$completeContent = preg_replace('/<a href="#" class="e_video_play">/', '<a href="#/" class="e_video_play" role="button" tabindex="0" on="tap:'.$matches[1][$i].$i.'.hide">', $completeContent, 1 );
			}

			//$completeContent = preg_replace('/<div class="e_video_overlay acss(.*?)">/', '<div class="e_video_overlay acss$1" id="$1">', $completeContent);

		   //$completeContent = preg_replace('/<a href="#" class="e_video_play">/', '<a href="#/" class="e_video_play" role="button" tabindex="0" on="tap:'.$matches[1].'.hide">', $completeContent);

           $completeContent = preg_replace('/<div class="e_video_overlay (.*?)" id="(.*?)">(.*?)<a href="#\/" class="(.*?)" role="button" tabindex="0" on="tap:\.hide"><\/a>/', '<div class="e_video_overlay $1" id="$2">$3<a href="#/" class="$4" role="button" tabindex="0" on="tap:$2.hide"></a>', $completeContent);
...
$VideoSliderItem = new AMP_PC_ET_Builder_Module_Video_Slider();//et_pb_video_slider_item
remove_shortcode( 'et_pb_video_slider_item' );
add_shortcode( 'et_pb_video_slider_item', array($VideoSliderItem, '_render'));
}