<?php
if(!class_exists('AMP_PC_ET_Builder_Module_Fullwidth_Map')){

class AMP_PC_ET_Builder_Module_Fullwidth_Map extends ET_Builder_Module {
	function init() {
	...
	function render( $attrs, $content = null, $render_slug='' ) {
		...
		$output = sprintf(
			'<div%5$s class="%6$s"%11$s>
				<amp-iframe title="Google map pin on Googleplex, Mountain View CA"
						  width="600"
                          height="400"
                          layout="responsive"
                          sandbox="allow-scripts allow-same-origin allow-popups"
                          frameborder="0"
						  data-mouse-wheel="%7$s" data-mobile-dragging="%8$s"
                          src="https://maps.google.com/maps?q=%1$s,%2$s&center=(%1$s,%2$s)&zoom=%3$d&output=embed">%8$s</amp-iframe></div>
				%4$s
			</div>',
...
$mapObj = new AMP_PC_ET_Builder_Module_Fullwidth_Map();
