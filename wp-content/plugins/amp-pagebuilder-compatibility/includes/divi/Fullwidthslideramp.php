...
if(!class_exists('AMP_PC_ET_Builder_Module_Fullwidth_Slider')){
class AMP_PC_ET_Builder_Module_Fullwidth_Slider extends ET_Builder_Module {
	function init() {
		//echo "Came";
	...
	public function css_style_fwamp(){
	echo '.em .e_slides .e_slideim amp-img{width: 300px;top:155px;}.em .e_slides amp-carousel{height:300px}@media only screen and (max-width: 600px){.e_slides amp-carousel .e_slideim amp-img{display:none;}}#page-container .e_sl .e_slide {margin-right: 0%;}';
	}	
	...
	function render( $attrs, $content = null, $render_slug='' ) {
		add_action('amp_post_template_css',array($this,'css_style_fwamp'));
		...
		$content = $this->content;
		$pattern = '/<div class="(.*?)" data-slide-id="(.*?)">/';
		$contentmatch = preg_match_all($pattern, $content,$matches);
		if(preg_match($pattern, $content)){
			$total_matches = $matches[2];
		    $matchescount = count($total_matches);
            $ampcarousel = '';
            $ampcarousel_close = '';
			if($matchescount !== 1){
					$ampcarousel = '<amp-carousel width="100" height="100" layout="responsive" type="slides" autoplay  delay="5000">';
					$ampcarousel_close = '</amp-carousel>';
			}
		}
...
$FullwidthSlider = new AMP_PC_ET_Builder_Module_Fullwidth_Slider();
remove_shortcode( 'et_pb_fullwidth_slider' );
add_shortcode( 'et_pb_fullwidth_slider', array( $FullwidthSlider, '_render' ));
}
