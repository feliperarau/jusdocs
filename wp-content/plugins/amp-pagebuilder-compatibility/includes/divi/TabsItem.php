<?php
if(!class_exists('AMP_PC_ET_Builder_Module_Tabs_Item')){
class AMP_PC_ET_Builder_Module_Tabs_Item extends ET_Builder_Module {
	function init() {
	...
	function render( $attrs, $content = null, $render_slug='' ) {
		...
		$output = sprintf(
			'<div class="amp_et_pb_tab_content %2$s">
				%4$s
				%3$s
				%1$s
			</div><!--et_pb_tab_end-->',
...
$accordianItemObj = new AMP_PC_ET_Builder_Module_Tabs_Item();//et_pb_accordion_item
remove_shortcode( 'et_pb_tab' );
add_shortcode( 'et_pb_tab', array($accordianItemObj, '_render'));
}