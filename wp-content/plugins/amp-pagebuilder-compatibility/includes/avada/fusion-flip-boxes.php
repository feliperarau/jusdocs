...
if ( fusion_is_element_enabled( 'fusion_flip_boxes' ) ) {
	...
	if ( ! class_exists( 'AMP_FusionSC_FlipBoxes' ) ) {
		/**
		 * Shortcode class.
		 *
		 ...
		 * @since 1.0
		 */
		class AMP_FusionSC_FlipBoxes extends Fusion_Element {
			...
			public function __construct() {
				...
				add_action('amp_post_template_css', array( $this,'amp_flip_box_custom_css' ) );

			}
            function amp_flip_box_custom_css(){
				$inline_styles = '.fu-flex-container .fu-row {
 ...
 * @since 1.0
 */
function amp_fusion_element_flip_boxes() {
	global $fusion_settings;
	if ( ! $fusion_settings ) {
...
add_action( 'fusion_builder_before_init', 'amp_fusion_element_flip_boxes' );

/**
 * Map shortcode to Fusion Builder
 */
function amp_fusion_element_flip_box() {
...
add_action( 'fusion_builder_before_init', 'amp_fusion_element_flip_box' );
