...
if ( fusion_is_element_enabled( 'fusion_gallery' ) ) {
	...
	if ( ! class_exists( 'AMP_PC_FusionSC_FusionGallery' ) ) {
		/**
		 * Shortcode class.
		 *
		 ...
		 * @since 1.0
		 */
		class AMP_PC_FusionSC_FusionGallery extends Fusion_Element {
			...
			public function __construct() {
				...
				add_action( 'amp_post_template_css', array( $this, 'amp_pc_fu_gallery_styles' ) );
			}
			public function amp_pc_fu_gallery_styles(){
					$no_of_cols = array( 1 => 'one',2 => 'two',3 => 'three',4 =>'four',5 =>'five',6 =>'six',7 =>'seven',8=>'eight',9=>'nine',10=>'ten');
					$cols = array();
					for($i=0;$i<$this->num_of_columns;$i++){
						$cols[] = '1fr';
					}
					$this->amp_gallery_styles = '.fusion-grid-'.$this->num_of_columns.' .fusion-grid-column, .fusion-grid-'.$no_of_cols[$this->num_of_columns].' .fusion-grid-column {
								    width: calc(100% / '.$this->num_of_columns.');
								}
								.fu-grid-'.$this->num_of_columns.' .fu-grid-column {
								    width: 100%;
								}
								.fu-grid-'.$this->num_of_columns.'{
								    display: inline-grid;
								    width: 100%;
								    grid-template-columns: '.implode(' ', $cols).';
								    grid-gap: 10px;
								}
	...
	remove_shortcode('fusion_gallery');
	new AMP_PC_FusionSC_FusionGallery();
 ...
 * @since 1.0
 */
function amp_fusion_element_gallery() {
	global $fusion_settings;
...
add_action( 'fusion_builder_before_init', 'amp_fusion_element_gallery' );

/**
 * Map shortcode to Fusion Builder.
 */
function amp_fusion_element_fusion_gallery_image() {
	fusion_builder_map(
		array(
...
add_action( 'fusion_builder_before_init', 'amp_fusion_element_fusion_gallery_image' );
