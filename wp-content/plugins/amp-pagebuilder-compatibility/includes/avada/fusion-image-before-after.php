...
if ( fusion_is_element_enabled( 'fusion_image_before_after' ) ) {
	...
	if ( ! class_exists( 'AMP_PC_FusionSC_ImageBeforeAfter' ) ) {
		/**
		 * Shortcode class.
		 *
		 ...
		 * @since 1.5
		 */
		class AMP_PC_FusionSC_ImageBeforeAfter extends Fusion_Element {
			 ...
			 * @since 1.5
			 */
			public $amp_img_before_after_styles = '';
			public function __construct() {
				...
				add_action( 'amp_post_template_css', array( $this,'amp_pc_fu_img_before_after_styles') );
				add_shortcode( 'fusion_image_before_after', array( $this, 'render' ) );

			}
			public function amp_pc_fu_img_before_after_styles(){
				echo $this->amp_img_before_after_styles;
			}
			/**
			...
			public function render( $args, $content = '' ) {
				...
				if ( '' !== $styles ) {
					$this->amp_img_before_after_styles = $styles;
				...
				$html .= '<amp-image-slider ' . FusionBuilder::attributes( 'image-before-after-shortcode' ) . ' width="300" height="200" layout="responsive" initial-slider-position="0.5" disable-hint-reappear>';
				if ( is_rtl() && 'vertical' !== $this->args['orientation'] ) {
					if ( isset( $this->args['after_image'] ) ) {

						$html .= '<amp-img ' . FusionBuilder::attributes( 'image-after-image' ) . ' layout="fill"></amp-img>';
					}

					if ( isset( $this->args['before_image'] ) ) {
						$html .= '<amp-img ' . FusionBuilder::attributes( 'image-before-image' ) . ' layout="fill"></amp-img>';
					}
				} else {
					if ( isset( $this->args['before_image'] ) ) {
						$html .= '<amp-img ' . FusionBuilder::attributes( 'image-before-image' ) . ' layout="fill"></amp-img>';
	...
	remove_shortcode('fusion_image_before_after');
	new AMP_PC_FusionSC_ImageBeforeAfter();
 ...
 * @since 1.0
 */
function amp_fusion_element_image_before_after() {
...
add_action( 'fusion_builder_before_init', 'amp_fusion_element_image_before_after' );
