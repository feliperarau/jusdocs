...
if ( fusion_is_element_enabled( 'fusion_modal' ) ) {
	...
	if ( ! class_exists( 'AMP_PC_FusionSC_Modal' ) ) {
		/**
		 * Shortcode class.
		 *
		 ...
		 * @since 1.0
		 */
		class AMP_PC_FusionSC_Modal extends Fusion_Element {
			...
			function add_amp_bind_script_to_modal_box($data){
			...
			public function render( $args, $content = '' ) {
				...
				$html  = '<div class="hide" [class]="myClass"><div ' . FusionBuilder::attributes( 'modal-shortcode' ) . '>';
				$html .= $style;
				...
				$html .= '<button role="button" tabindex="-1" on="tap:AMP.setState({ myClass: \'hide\' })" ' . FusionBuilder::attributes( 'modal-shortcode-button' ) . '>&times;</button>';
				...
				if ( 'yes' == $show_footer ) {
					$html .= '<div role="button" tabindex="-1" on="tap:AMP.setState({ myClass: \'hide\' })"' . FusionBuilder::attributes( 'modal-footer' ) . '>';
				...
				$this->modal_counter++;
				add_filter('amp_post_template_data',array( $this,'add_amp_bind_script_to_modal_box'));
				return $html;
 ...
 * @since 1.0
 */
function amp_fusion_element_modal() {
...
add_action( 'fusion_builder_before_init', 'amp_fusion_element_modal' );

/**
 * Map shortcode to Fusion Builder
 */
function amp_fusion_element_modal_link() {
	fusion_builder_map(
		array(
...
add_action( 'fusion_builder_before_init', 'amp_fusion_element_modal_link' );