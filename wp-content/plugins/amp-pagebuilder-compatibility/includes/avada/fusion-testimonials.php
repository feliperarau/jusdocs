...
if ( fusion_is_element_enabled( 'fusion_testimonials' ) ) {
	...
	if ( ! class_exists( 'AMP_PC_FusionSC_Testimonials' ) ) {
		/**
		 * Shortcode class.
		 *
		 ...
		 * @since 1.0
		 */
		class AMP_PC_FusionSC_Testimonials extends Fusion_Element {
			 ...
			 * @since 1.0
			 */
			public $amp_fu_testminials_styles = '';
			public function __construct() {
				...
				add_action( 'amp_post_template_css', array( $this, 'amp_pc_fu_testimonials_styles' ) );

			}
			public function amp_pc_fu_testimonials_styles(){
				$this->amp_fu_testminials_styles .= '
					.futml .testimonials-shortcode-blockquote q{overflow-y:scroll;height:185px;}
					@media only screen and (min-width: 768px) {
						.fusion-testimonials.clean blockquote q{overflow-y:scroll;height:185px;}
					}
				...
				echo $this->amp_fu_testminials_styles;
			}
			...
			public function render_parent( $args, $content = '' ) {
				...
				$styles .= '.fusion-testimonials.fusion-testimonials-'.$this->testimonials_counter.' .review{display:block;}';
				//$styles .= '</style>';
				
				$this->amp_fu_testminials_styles .= $styles;
				$pagination = '';
				...
				$html = sprintf(
					'<div %s>%s<amp-carousel height="400" layout="fixed-height" type="slides" %s>%s</amp-carousel>%s</div>',
			...
			private function render_child_clean( $content ) {
				...
				$author .= '</span></div>';
				preg_match('/style="(.*?)"/', FusionBuilder::attributes( 'testimonials-shortcode-quote' ),$matches);
				$testimonials_css = $matches[1];
				$styles .= '.fusion-testimonials.fusion-testimonials-'.$this->testimonials_counter.'  .testimonials-shortcode-blockquote q{'.$testimonials_css.'}';
				$this->amp_fu_testminials_styles .= $styles;
	...
	remove_shortcode('fusion_testimonials');
	new AMP_PC_FusionSC_Testimonials();
 ...
 * @since 1.0
 */
function amp_fusion_element_testimonials() {
...
add_action( 'fusion_builder_before_init', 'amp_fusion_element_testimonials' );
 ...
 * @since 1.0
 */
function amp_fusion_element_testimonial() {
	fusion_builder_map(
		array(
...
add_action( 'fusion_builder_before_init', 'amp_fusion_element_testimonial' );
