...
if ( fusion_is_element_enabled( 'fusion_tabs' ) ) {
	...
	if ( ! class_exists( 'AMP_PC_FusionSC_Tabs' ) ) {
		/**
		 * Shortcode class.
		 *
		 ...
		 * @since 1.0
		 */
		class AMP_PC_FusionSC_Tabs extends Fusion_Element {
			...
			public function __construct() {
				...
				add_action('amp_post_template_css', array( $this,'amp_pb_avda_stl') );
				add_shortcode( 'fusion_tabs', array( $this, 'fusion_tabs' ) );
				add_shortcode( 'fusion_tab', array( $this, 'fusion_tab' ) );

			}

			public function amp_pb_avda_stl(){
				echo '.fu-tabs .tab-pane { word-spacing: 8px;}[class^="icon-"], [class*=" icon-"] {font-family: "Poppins",sans-serif;}';
			}
			...
			public function render_parent( $args, $content = '' ) {
				...
				$html = '<amp-script><div ' . FusionBuilder::attributes( 'tabs-shortcode' ) . '>' . $styles . '<div ' . FusionBuilder::attributes( 'nav' ) . '><ul ' . FusionBuilder::attributes( 'nav-tabs' . $justified_class ) . '>';
				...
				for ( $i = 0; $i < $tabs_count; $i++ ) {
					...
					if ( $is_first_tab ) {
						$tab_nav      = '<li ' . FusionBuilder::attributes( 'active' ) . '><a ' . FusionBuilder::attributes( 'tabs-shortcode-link', array( 'index' => $i ) ) . '><h4 ' . FusionBuilder::attributes( 'fusion-tab-heading' ) . '>' . $tab_title . '</h4></a></li></amp-script>';
						$is_first_tab = false;
					} else {
				...
				$html .= '</ul></div><div ' . FusionBuilder::attributes( 'tab-content' ) . '>' . $tab_content . '</div></div></amp-script>';
 ...
 * @since 1.0
 */
function fusion_element_tabs_amp() {
...
add_action( 'fusion_builder_before_init', 'fusion_element_tabs_amp' );
 ...
 * @since 1.0
 */
function fusion_element_tab_amp() {
	fusion_builder_map(
		array(
...
add_action( 'fusion_builder_before_init', 'fusion_element_tab_amp' );
