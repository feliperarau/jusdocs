...
if ( function_exists( 'fusion_is_element_enabled' ) && fusion_is_element_enabled( 'fusion_portfolio' ) ) {
	...
	if ( ! class_exists( 'AMP_PC_FusionSC_Portfolio' ) && class_exists( 'Fusion_Element' ) ) {
		/**
		 * Shortcode class.
		 *
		 * @package fusion-core
		 * @since 1.0
		 */
		class AMP_PC_FusionSC_Portfolio extends Fusion_Element {
			...
			private $post_id;
			public static $args;
			public $amp_fusion_portfolio_styles = '';
			public function __construct() {
				...
				add_action( 'amp_post_template_css', [ $this, 'amp_pc_fu_portfolio_styles' ],99 );
				add_action( 'amp_post_template_css', [ $this, 'amp_pc_fu_portfolio_custom_styles' ] );
			...
			public function amp_pc_fu_portfolio_styles(){
				echo $this->amp_fusion_portfolio_styles;
			}
			...
			public function render( $args, $content = '' ) {
				...
				if ( 'carousel' !== self::$args['layout'] && 'no_text' !== self::$args['text_layout'] ) {
					...
					$this->amp_fusion_portfolio_styles .= $styling;
				}
				...
				while ( $portfolio_query->have_posts() ) {
					...
					if ( has_post_thumbnail() || $fusion_settings->get( 'featured_image_placeholder' ) || fusion_get_page_option( 'video', $this->post_id ) ) {
						...
						if ( 'carousel' === $layout ) {
							...
							$portfolio_posts .= '<div ' . FusionBuilder::attributes( 'fusion-carousel-item-wrapper' ) . '>' . avada_render_rich_snippets_for_pages() . $image . $title_terms . '</div>';
							...
							if ( 'carousel' !== self::$args['layout'] && 'no_text' !== self::$args['text_layout'] ) {
								...
								$loop_content = strip_shortcodes( $loop_content );

								$stripped_content = '<p class="exrpt">'. wp_trim_words(  $loop_content, 25 ) .'</p>';
				...
				if ( 'carousel' === $layout ) {
					...
					$main_carousel = '<amp-carousel id="fus-pofo-crsl" class="crsl-img-media" height="500" layout="fixed-height" type="carousel" controls>'. $portfolio_posts . '</amp-carousel>';

					$html = '<div ' . FusionBuilder::attributes( 'portfolio-shortcode' ) . '>' . $main_carousel . '</div>';
					...
					// Put it all together.
					$html = $styling . '<div ' . FusionBuilder::attributes( 'portfolio-shortcode' ) . 'id="wrapper">' . $filter_wrapper . $styles . '<div ' . FusionBuilder::attributes( 'portfolio-shortcode-portfolio-wrapper' ) . '>' . $portfolio_posts . '</div>' . $pagination . '</div>';

				}

				$this->amp_fusion_portfolio_styles .= $styles;
			...
			public function amp_pc_fu_portfolio_custom_styles(){
		              if ( $column_spacing ) {
		              $styles = '.fusion-portfolio-' . $this->portfolio_counter . ' .fusion-portfolio-wrapper .fusion-col-spacing{padding:' . ( $column_spacing / 2 ) . 'px;}';
		               }  
		               echo $styles;
		            }
		}
