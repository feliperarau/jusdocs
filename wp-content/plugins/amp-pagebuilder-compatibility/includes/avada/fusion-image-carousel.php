    ...
    if ( ! class_exists( 'AMP_PC_FusionSC_ImageCarousel' ) ) {
        ...
        class AMP_PC_FusionSC_ImageCarousel extends Fusion_Element {
            ...
            function print_css_data(){
                $col_per = round(100/$this->parent_args['columns']);
                $col_space = $this->parent_args['column_spacing'];
                if($this->parent_args['columns'] == 1){
                    $col_space = 0;
                }
                echo '.fusion-carousel amp-carousel .slide {
                    width: '.$col_per.'%;
                    margin-right: '.$col_space.'px;
                }';
            }
            ...
            public function render_parent( $args, $content = '' ) {
                ...
                // The main carousel.
                $html .= '<amp-carousel class="carousel2" '.$width.'
                layout="'.$layout.'"
                height="'.$height.'"
                type="'.$type.'" ' . FusionBuilder::attributes( 'fusion-carousel-holder' ) . '>';
 ...
 */
function amp_fusion_element_images() {
    fusion_builder_map(
        array(
...
add_action( 'fusion_builder_before_init', 'amp_fusion_element_images' );

/**
 * Map shortcode to Fusion Builder.
 */
function amp_fusion_element_fusion_image() {
    fusion_builder_map(
        array(
...
add_action( 'fusion_builder_before_init', 'amp_fusion_element_fusion_image' );
