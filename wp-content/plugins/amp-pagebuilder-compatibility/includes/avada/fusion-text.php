...
if ( fusion_is_element_enabled( 'fusion_text' ) ) {
	...
	if ( ! class_exists( 'AMP_PC_FusionSC_FusionText' ) ) {
		/**
		 * Shortcode class.
		 *
		 ...
		 * @since 1.0
		 */
		class AMP_PC_FusionSC_FusionText extends Fusion_Element {
			...
			public function render( $args, $content = '' ) {
				...
				if(preg_match('/<h(\d)\s(.*?)data-fontsize="(.*?)"\sdata-lineheight="(.*?)"(.*?)>/', $content)){
					$html = preg_replace('/<h(\d)\s(.*?)data-fontsize="(.*?)"\sdata-lineheight="(.*?)"(.*?)>/', '<h$1 $2 style="font-size:$3px;line-height:1.3"$5>', $content);
				}
	...
	remove_shortcode( 'fusion_text' );
	new AMP_PC_FusionSC_FusionText();
 ...
 * @since 1.0
 */
function amp_fusion_element_text() {
	global $fusion_settings;
...
add_action( 'fusion_builder_before_init', 'amp_fusion_element_text' );
