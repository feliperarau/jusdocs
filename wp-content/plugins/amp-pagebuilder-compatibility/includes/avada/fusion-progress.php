...
if ( fusion_is_element_enabled( 'fusion_progress' ) ) {
	...
	if ( ! class_exists( 'AMP_PC_FusionSC_Progressbar' ) ) {
		/**
		 * Shortcode class.
		 *
		 ...
		 * @since 1.0
		 */
		class AMP_PC_FusionSC_Progressbar extends Fusion_Element {
			 ...
			 * @since 1.0
			 */
			public $amp_progress_bar_styles = '';
			public function __construct() {
				...
				add_action( 'amp_post_template_css', array( $this, 'amp_pc_progress_bar_styles' ) );
			}
			public function amp_pc_progress_bar_styles(){
				echo $this->amp_progress_bar_styles;
			}
			...
			public function render( $args, $content = '' ) {
				...
				$progress_bar_css = FusionBuilder::attributes( 'progressbar-shortcode-content' );
				preg_match('/style="(.*?)"/', $progress_bar_css,$matches);
				$dynamic_css = $matches[1];
				...
				$styles .= '.fusion-progressbar .fusion-progressbar-bar .progress-bar-'.$this->progress_bar_counter.'{'.$dynamic_css.'}';
			
				$this->amp_progress_bar_styles .= $styles;
	...
	remove_shortcode('fusion_progress');
	new AMP_PC_FusionSC_Progressbar();
}
 ...
 * @since 1.0
 */
function amp_fusion_element_progress() {
...
add_action( 'fusion_builder_before_init', 'amp_fusion_element_progress' );
