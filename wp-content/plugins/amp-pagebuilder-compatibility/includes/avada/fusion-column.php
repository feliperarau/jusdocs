...
if ( ! class_exists( 'AMP_PC_FusionSC_Column' ) ) {
	/**
	 * Shortcode class.
	 *
	 ...
	 * @since 1.0
	 */
	class AMP_PC_FusionSC_Column extends Fusion_Element {
		 ...
		 * @since 1.0
		 */
		public $amp_column_styles = array();
		public $amp_column_wrapper_styles = '';
		public function __construct() {
			parent::__construct();
			add_shortcode( 'fusion_builder_column', array( $this, 'render' ) );
			add_action( 'amp_post_template_css', array( $this, 'amp_pc_column_styles' ) );
		}

		public function amp_pc_column_styles(){
			echo $this->amp_column_wrapper_styles;
			echo implode('',array_unique($this->amp_column_styles));
		}
		...
		public function render( $atts, $content = '' ) {
			...
			$this->amp_column_styles[] = '.fulcol.fusion_builder_column_'.$type.'{ width:'.$map_old_spacing[ strval( $column_size ) ].';}
				@media only screen and (max-width: 768px) {
					.fulcol.fusion_builder_column_'.$type.'{
						max-width:100%;
						width:100%;
						padding:10px;
					}
			...
			if ( ( 'none' === $hover_type && empty( $link ) ) || ( empty( $hover_type ) && empty( $link ) ) ) {
				...
				$this->amp_column_wrapper_styles .= '.fusion_builder_column_'.$type.' .'.$box_shadow_class.'{'.$wrapper_style . $wrapper_style_bg . $bg_color_fix.'}';
				...
				$this->amp_column_wrapper_styles .= '.fusion_builder_column_'.$type.' .'.$box_shadow_class.'{'.$wrapper_style.'}';
