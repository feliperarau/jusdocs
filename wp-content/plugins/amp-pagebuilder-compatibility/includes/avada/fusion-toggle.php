...
if ( fusion_is_element_enabled( 'fusion_accordion' ) ) {
	...
	if ( ! class_exists( 'AMP_PC_FusionSC_Toggle' ) ) {
		/**
		 * Shortcode class.
		 *
		 ...
		 * @since 1.0
		 */
		class AMP_PC_FusionSC_Toggle extends Fusion_Element {
			...
			public function __construct() {
				...
				add_action('amp_post_template_css', array( $this,'amp_pb_avada_fusion_toggle' ) );

			}

			public function amp_pb_avada_fusion_toggle(){
				$inline_styles = '.fusion-accordian .fusion-panel{
				    border-bottom: 1px solid #e0dede;
				    margin-bottom: 15px;
				}
				.fu-accordian .panel-title{
				    background: transparent;
				    border: none;
				    padding-bottom: 10px;
				}
				.fu-accordian .panel-title span:hover{
				    color: #65bc7b;
				}
				...
				amp-accordion section[expanded] .panel-title .fa-fusion-box:before{
				    content: "\e61c";
				}
				.fu-toggle-icon-wrapper{
				    background:#515151;
				    width: 22px;
				    text-align: center;
				    border: none;
				}
				.acc-cnt{ display: flex;  align-items: center;}';
				echo $inline_styles;
			}
			...
			public function render_parent( $args, $content = '' ) {
				...
				$html = sprintf(
					'%s<div %s><amp-accordion %s expand-single-section>%s</amp-accordion></div>',
					$style_tag,
 ...
 * @since 1.0
 */
function amp_pc_fusion_element_accordion() {
...
add_action( 'fusion_builder_before_init', 'amp_pc_fusion_element_accordion' );
 ...
 * @since 1.0
 */
function amp_pc_fusion_element_toggle() {
	fusion_builder_map(
		array(
...
add_action( 'fusion_builder_before_init', 'amp_pc_fusion_element_toggle' );
