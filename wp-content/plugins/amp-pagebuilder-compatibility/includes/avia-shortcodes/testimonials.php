...
if ( !class_exists( 'amp_avia_sc_testimonial' ) )
{
	class amp_avia_sc_testimonial extends aviaShortcodeTemplate
	{
			static $columnClass;
			static $rows;
			static $counter;
...
array(
			...
			function shortcode_handler($atts, $content = "", $shortcodename = "", $meta = "")
			{
				...
				amp_avia_sc_testimonial::$counter = 1;
				amp_avia_sc_testimonial::$rows = 1;
				amp_avia_sc_testimonial::$columnClass = $columnClass;
				amp_avia_sc_testimonial::$columns = $columns;
				amp_avia_sc_testimonial::$style = $style;
				amp_avia_sc_testimonial::$grid_style = $grid_style;



				//if we got a slider we only need a single row wrpper
				if($style != "grid") amp_avia_sc_testimonial::$columns = 100000;

				$output .= ShortcodeHelper::avia_remove_autop($content, true);

				//close unclosed wrapper containers
				if(amp_avia_sc_testimonial::$counter != 1){
				$output .= "</section>";
				}
			...
			function av_testimonial_single($atts, $content = "", $shortcodename = "")
			{
				...
				$output = "";
				$avatar = "";
				$grid = amp_avia_sc_testimonial::$style == 'grid' ? true :false;
				$grid_style = $grid == true? amp_avia_sc_testimonial::$grid_style : "";
				$class = amp_avia_sc_testimonial::$columnClass." avia-testimonial-row-".amp_avia_sc_testimonial::$rows." ";
				//if(count($testimonials) <= $rows * $columns) $class.= " avia-testimonial-row-last ";
				if(amp_avia_sc_testimonial::$counter == 1) $class .= "avia-first-testimonial";
				if(amp_avia_sc_testimonial::$counter == amp_avia_sc_testimonial::$columns) $class .= "avia-last-testimonial";
				...
				if(amp_avia_sc_testimonial::$counter == 1)
                {
				    ...
				    $output .= '<amp-carousel width="400" height="300" layout="responsive" type="slides" autoplay delay="10000">';
				}
...
if($subtitle)	$output .= 					"<span  class='avia-testimonial-subtitle {$this->subtitle_class}' {$this->title_styling}  {$markup_job}>{$subtitle}</span>";
	...
	if($link)	$output .= 					"<a class='aviablank avia-testimonial-link' href='{$link}' >{$linktext}</a>";
				...
				if(amp_avia_sc_testimonial::$counter == amp_avia_sc_testimonial::$columns)
                {
				    $output .= "</section>";
				    $output .= "</amp-carousel>";
				}

				amp_avia_sc_testimonial::$counter++;
				if(amp_avia_sc_testimonial::$counter > amp_avia_sc_testimonial::$columns) { amp_avia_sc_testimonial::$counter = 1; amp_avia_sc_testimonial::$rows++; }
