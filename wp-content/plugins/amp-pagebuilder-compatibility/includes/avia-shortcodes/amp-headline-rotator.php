...
if ( !class_exists( 'amp_avia_sc_headline_rotator' ) )
{
	class amp_avia_sc_headline_rotator extends aviaShortcodeTemplate
	{
			var $count;
			/**
			...
			public function amp_pc_headline_rotator_styles(){
		    $inline_styles= ".av-rotator-container amp-carousel{
    						height: 85px;
    						max-width: 420px;}
    						.av-rotator-container .av-rotator-text .amp-carousel-button{display:none;}
							";
		    echo $inline_styles;
			}	
			/**
			...
			{
				add_action('amp_post_template_css',array($this,'amp_pc_headline_rotator_styles'));
			    ...
			    preg_match_all("/\[av_rotator_item(.*?)title='(.*?)'\s+link='(.*?)'\s+linktarget='(.*?)'(.*?)\]/", $content, $matches);
				$initial = 1;
				$module = '';
				foreach ($matches[2] as $key => $value) {
					...
					if(!empty($matches[3][$key])){
                        $h_tag = 'a';
                        $src_url = explode(',', $matches[3][$key]);
                        if($src_url[0] == 'manually'){
                        	$src = $src_url[1];
                        }else{
                        	$src = get_permalink((int)$src_url[1]);
                        }
                        if($matches[4][$key] == '_blank'){
                            $target = 'target = "_blank"';
                        }
                        $href = 'href="'.$src.'"';
					}
                  	$module .= '<div>
						      <'.$h_tag.'  '.$href.' '.$target.' class="av-rotator-text-single av-rotator-text-single-'.esc_attr($initial).'">'.esc_html($value).'</'.$h_tag.'>
						     </div>';
					$initial++;
				}
				$output .= ' <amp-carousel autoplay delay= "2000" width="50" height="50" layout="responsive" 		  type="slides" role="region" aria-label="Carousel with arbitrary HTML content"> 
                             '.$module.'
                        	 <span class="amp-carousel-button"> </span>
							 </amp-carousel>';
				$output .= "</span>";
