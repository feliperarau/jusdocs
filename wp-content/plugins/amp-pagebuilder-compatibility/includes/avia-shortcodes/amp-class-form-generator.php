...
if( ! class_exists( 'amp_avia_form' ) )
{
	class amp_avia_form
	{
		...
		{
			//add_action( 'wp_ajax_amp_avia_optin_newsletter_submit', array('AMP_PC_Divi_Pagebuidler', 'amp_avia_optin_newsletter_submit') );

            //add_action( 'wp_ajax_nopriv_amp_avia_optin_newsletter_submit', array('AMP_PC_Divi_Pagebuidler', 'amp_avia_optin_newsletter_submit') );

			add_filter('avf_safe_string_trans', array(&$this,'remove_invalid_chars'), 10, 3);

			$this->form_params = $params;
			$this->formID 		= amp_avia_form::$form_id ++;
			...
			$schema_class_append = function_exists('saswp_schema_markup_output') ? "saswp-review-submission-form" : "";

            $actionXhrUrl = preg_replace('#^http?:#', '', $params['action']);
			$this->output  = '<form action-xhr="'.ampforwp_url_controller($actionXhrUrl).'" method="post" '.$form_data.' class="'.$schema_class_append.' '.$form_class.' '.$extraClass.'" data-avia-form-id="'.$this->formID.'" '.$redirect.' id="amp-avia-form"><fieldset id="amp_avia_form">';
		...
		function display_form($return = false)
		{
			...
			if($this->submit_form && $call_instance->$call_function( $this ) && empty($this->error_msg))
			{
				...
				$this->form_params['success'] = strip_tags($this->form_params['success']);
				echo json_encode(array('success_msg' =>  $this->form_params['success'] ));
				die;
			}
			else
			{
			...
			$this->output .= '</fieldset><div submit-success>
    ...
    <div submit-error>
      <template type="amp-mustache">
		...
		function text($id, $element, $type = 'text')
		{
            ...
            $required_amp_fields = '';
			if(!empty($element['check'])){
               $required_amp_fields = 'required';
			}
			
			$form_el = ' <input name="'.$id.'" class="text_input '.$element_class.'" type="'.$type.'" id="'.$id.'" value="'.$value.'" '.$placeholder.' '.$required_amp_fields.'/>';
        ...
        function datepicker($id, $element)
        {
            ...
            $this->elements_html .= "<p class='".$p_class.$element['class']."' id='element_$id'>";
            $form_el = ' <input name="'.$id.'" class="avia_datepicker text_input '.$element_class.'" type="text" id="'.$id.'" value="'.$value.'" placeholder="'.$placeholder.'" '.$required_amp_fields.' />';
            $label = '<label for="'.$id.'">'.$element['label'].$required.'</label>';
		...
		{   $required_amp_fields = '';
			...
			$this->elements_html .= "<p class='".$p_class.$element['class']."' id='element_$id'>";
			$this->elements_html .= '    <input '.$checked.' name="'.$id.'" class="input_checkbox '.$element_class.'" type="checkbox" id="'.$id.'" value="true" '.$required_amp_fields.'/><label class="input_checkbox_label" for="'.$id.'">'.$element['label'].$required.'</label>';
			$this->elements_html .= "</p>";
		}
		...
		{   $required_amp_fields = '';
			...
			$this->elements_html .= "<p class='".$p_class.$element['class']."' id='element_$id'>";
			$form_el = ' <select '.$multi.' name="'.$id.'" class="select '.$element_class.'" id="'.$id.'" '.$required_amp_fields.'>'.$select.'</select>';
			$label = '<label for="'.$id.'">'.$element['label'].$required.'</label>';
		...
		{   $required_amp_fields = '';
			...
			$this->elements_html .= '	 <textarea '.$placeholder.' name="'.$id.'" class="text_area '.$element_class.'" cols="40" rows="7" id="'.$id.'" '.$required_amp_fields.'>'.$value.'</textarea>';
			$this->elements_html .= "</p>";
		}
