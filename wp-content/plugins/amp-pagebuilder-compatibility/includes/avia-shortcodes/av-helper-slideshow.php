...
if ( ! class_exists( 'amp_avia_slideshow' ) )
{
	class amp_avia_slideshow
	{
		...
		public function html()
		{
			...
			$html .= "<div class='avia-slideshow-".amp_avia_slideshow::$slider." {$extraClass} avia-slideshow-".$this->config['size']." ".$this->config['handle']." ".$this->config['class']." avia-".$this->config['animation']."-slider ' $markup>";
			
			$html .= '<amp-carousel class="{$cond_play_class}" layout="responsive" height="400" width="500" type="slides" {$style} >';
			$html .= $slide_html;
			$html .= '</amp-carousel';
		...
		protected function advanced_slide()
		{
			...
			{				
				...
				if(isset($this->slides[$id]) || $slide_type == 'video')
				{
					...
					if($slide_type == 'video')
					{
						$this->service    = amp_avia_slideshow_video_helper::which_video_service($video);
						$video 			  = amp_avia_slideshow_video_helper::set_video_slide($video, $this->service, $meta , $this->config); 
					...
					if(!empty($img[0]))
					{
						$slider_data .= $this->config['bg_slider'] == 'true' ? "style='background-position:{$position};background-image:url(".$img[0].");background-size: contain;' data-img-url='".$img[0]."'" : '';
					...
					$html .= "<div {$slider_data} class='{$extra_class} slide-{$counter} ' >";
					$html .= "<".$tags[0]." data-rel='slideshow-".amp_avia_slideshow::$slider."' class='{$fallback_img_class}' {$fallback_img_style} {$linkdescription} >{$caption}";
		...
		protected function extract_subslides( array $slide_array )
		{
			...
			foreach($slide_array as $key => $slide)
			{
				...
				if( empty($slide['attr']['id']) && ! empty( $slide['attr']['video']) && $slide['attr']['slide_type'] === 'video')
				{
					...
					if( amp_avia_slideshow_video_helper::is_extern_service( $slide['attr']['video'] ) )
					{
...
if ( !class_exists( 'amp_avia_slideshow_video_helper' ) )
{
	class amp_avia_slideshow_video_helper
	{
		...
		static function set_video_slide( $video_url, $service = false, $meta = false, $config = false )
		{
			...
			switch( $service )
			{
				...
				case 'youtube':
					...
					$video 	= "<amp-youtube width='480' height='270' layout='responsive' data-videoid='".$video_id."' autoplay></amp-youtube>";
		...
		{
			$ervice = amp_avia_slideshow_video_helper::which_video_service($video_url);
			
			return in_array( $ervice, amp_avia_slideshow_video_helper::$extern_services );
		}
	}
}
