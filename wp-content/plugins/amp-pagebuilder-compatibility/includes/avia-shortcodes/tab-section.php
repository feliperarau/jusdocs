...
if ( ! class_exists( 'amp_avia_sc_tab_section' ) )
{
	//load the subsection shhortcode
	include_once( AMP_PAGEBUILDER_COMPATIBILITY_PLUGIN_DIR.'includes/avia-shortcodes/tab_sub_section.php');
	
	class amp_avia_sc_tab_section extends aviaShortcodeTemplate
	{
		...
		function editor_element( $params )
		{
			...
			amp_avia_sc_tab_section::$tab = 0;
			amp_avia_sc_tab_section::$tab_titles = array();
			amp_avia_sc_tab_section::$admin_active = ! empty( $args['av_admin_tab_active'] ) ? $args['av_admin_tab_active'] : 1;
			...
			//create tabs
			for( $i = 1; $i <= amp_avia_sc_tab_section::$tab; $i ++ )
			{
				$active_tab = $i == amp_avia_sc_tab_section::$admin_active ? 'av-admin-section-tab-active' : '';
				$tab_title = isset( amp_avia_sc_tab_section::$tab_titles[$i] ) ? amp_avia_sc_tab_section::$tab_titles[$i] : '';
		...
		public function amp_tab_section_styles(){
			$inline_styles = '/*Tab Section Avia Builder*/
						...
						.av-tab-section-inner-container .iconbox_top a.iconbox_icon:hover{
						    background-color:#ffffff;
						    color:#ed1d24;
						}';
			echo $inline_styles;
		...
		function shortcode_handler( $atts, $content = '', $shortcodename = '', $meta = '' )
		{
			...
			add_filter('amp_post_template_css',[$this,'amp_tab_section_styles'],999);
			...
			for( $i = 1; $i <= avia_sc_tab_section::$tab; $i ++ )
			{
				...
				//$tabs  .= "<a href='#{$tab_link}' data-av-tab-section-title='{$i}' class='av-section-tab-title {$active_tab} {$extraClass} '>{$icon}{$image}<span class='av-outer-tab-title'><span class='av-inner-tab-title'>{$tab_title}</span></span>{$arrow}</a>";;tap:AMP.setState({ activeTab: '.$i.' });
				$tabs .= '<span role="button" tabindex="'.($i-1).'" on="tap:amp-av-tab-section-'.avia_sc_tab_section::$count.'.toggle(index='.($i-1).', value=true),AMP.setState({ activeTab: '.$i.' })" [class]="activeTab == '.$i.'? \'av-section-tab-title av-active-tab-title '.$extraClass.'\':\'av-section-tab-title '.$active_tab.' '.$extraClass.'\'" class="av-section-tab-title '.$active_tab.' '.$extraClass.' ">'.$icon.''.$image.'<span class="av-outer-tab-title"><span class="av-inner-tab-title">'.$tab_title.'</span></span>'.$arrow.'</span>';
			}
			...
			$output .= '<amp-selector id="amp-av-tab-section-'.avia_sc_tab_section::$count.'" class="tabs-with-flex" role="tablist" keyboard-select-mode="focus">';
			$output .= $final_content;
			$output .= '</amp-selector>';
			$output .= '</div>';
