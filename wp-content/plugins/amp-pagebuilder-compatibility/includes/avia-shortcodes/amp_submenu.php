...
if ( !class_exists( 'amp_avia_sc_submenu' ) ) 
{
	class amp_avia_sc_submenu extends aviaShortcodeTemplate
	{
			static $count = 0;
			...
			function shortcode_handler($atts, $content = "", $shortcodename = "", $meta = "")
			{
				...
				if($atts['mobile'] == 'disabled'){
                   add_action('amp_post_template_css','amp_submenu_ccss',999);
				}
				
				extract($atts);
				$output  	= "";
				$sticky_div = "";
				amp_avia_sc_submenu::$count++;
				amp_avia_sc_submenu::$custom_items = 0;
				
				
				$params['class'] = "av-submenu-container {$color} ".$meta['el_class'];
				$params['open_structure'] = false; 
				$params['id'] = "sub_menu".amp_avia_sc_submenu::$count;
				$params['custom_markup'] = $meta['custom_markup'];
				$params['style'] = "style='z-index:".(amp_avia_sc_submenu::$count + 300)."'";
				...
				if($which_menu == "custom")
				{
					...
					{
						$element .= "<div class='hide' id='sh-dtop-amp' [class]='myClass'><ul id='av-custom-submenu-".avia_sc_submenu::$count."' class='av-subnav-menu av-submenu-pos-{$position}'>";
						$element .= $custom_menu;
						$element .= "</ul><div>";
					}
				}
				...
				$submenu_hidden = ""; 
				$mobile_button = $mobile == "active" ? "<a role='button' tabindex='-1' on='tap:AMP.setState({ myClass: \"show\" })' href='#' class='mobile_menu_toggle' ".av_icon_string('mobile_menu')."><span role='button' tabindex='-1' on='tap:AMP.setState({ myClass: \"hide\" })' class='av-current-placeholder'>".__('Menu', 'avia_framework')."</span></a>" : "";
				...
				$output .= "<div class='container av-menu-mobile-{$mobile} {$submenu_hidden}'>{$mobile_button}<div class='hide' id='sh-dtop-amp' [class]='myClass'>".$element."</div></div>";
			...
			function av_submenu_item($atts, $content = "", $shortcodename = "", $meta = "")
			{
                ...
                if(!empty($title))
                {
	                amp_avia_sc_submenu::$custom_items++;
	                ...
	                $output .= "<li class='menu-item menu-item-top-level {$button_style} menu-item-top-level-".amp_avia_sc_submenu::$custom_items."'>";
	        ...
	        function amp_submenu_ccss(){
				echo " #sh-dtop-amp { display: block;}";
			}
}
