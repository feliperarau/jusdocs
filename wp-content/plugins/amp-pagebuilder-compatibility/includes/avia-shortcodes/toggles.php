...
if( ! class_exists( 'amp_avia_sc_toggle' ) )
{
	class amp_avia_sc_toggle extends aviaShortcodeTemplate
	{
		/**
		 *
		 * @var int 
		...
		function shortcode_handler( $atts, $content = '', $shortcodename = '', $meta = '' )
		{
          ...
          if($atts['styling'] == ''){
          	add_action('amp_post_template_css','amp_avia_toggles_css',999);
          }
			...
			$output  = '<div ' . $markup . ' ' . $meta['custom_el_id'] . ' class="togglecontainer ' . $av_display_classes . ' ' . $styling . ' ' . $addClass . $meta['el_class'] . '" ' . $current_colors_attr . '><amp-accordion animate expand-single-section>';

			amp_avia_sc_toggle::$counter = 1;
			amp_avia_sc_toggle::$initial = $initial;
			amp_avia_sc_toggle::$tags = array();
			amp_avia_sc_toggle::$atts = $atts;

			$content  = ShortcodeHelper::avia_remove_autop( $content, true );
			$sortlist = ! empty( $sort ) ? $this->sort_list( $atts ) : '';

			$output .= $sortlist . $content . '</amp-accordion></div>';
		...
		public function av_toggle( $atts, $content = '', $shortcodename = '' )
		{
			...
			if( is_numeric( amp_avia_sc_toggle::$initial ) && amp_avia_sc_toggle::$counter == amp_avia_sc_toggle::$initial )
			{
			...
			{
				$toggle_atts['title'] = amp_avia_sc_toggle::$counter;
			}

			$setting_id = Avia_Builder()->get_developer_settings( 'custom_id' );
			if( empty( $toggle_atts['custom_id'] ) || in_array( $setting_id, array( 'deactivate' ) ) )
			{
				$toggle_atts['custom_id'] = 'toggle-id-' . amp_avia_sc_toggle::$toggle_id++;
			}
			else
			{
			...
			if( ! empty( amp_avia_sc_toggle::$atts['colors'] ) && amp_avia_sc_toggle::$atts['colors'] == 'custom' )
			{
				if( ! empty( amp_avia_sc_toggle::$atts['background_color'] ) )
				{
					$colors = 'background-color: ' . amp_avia_sc_toggle::$atts['background_color'] . '; ';
				}
	            
				if( ! empty(amp_avia_sc_toggle::$atts['font_color'] ) )
				{
					$colors .= 'color: ' . amp_avia_sc_toggle::$atts['font_color'] . '; ';
					$icon_color = "style='border-color:" . amp_avia_sc_toggle::$atts['font_color'] . ";'";
					$inherit .= ' av-inherit-font-color ';
					$titleClass .= ' hasCustomColor';
				}
	            
				if( ! empty( amp_avia_sc_toggle::$atts['border_color'] ) )
				{
					$colors .= 'border-color: ' . amp_avia_sc_toggle::$atts['border_color'] . '; ';
			...
			// hover styling
			$hover_styling = '';
			if( ! empty( amp_avia_sc_toggle::$atts['hover_colors'] ) && amp_avia_sc_toggle::$atts['hover_colors'] == 'custom' )
			{
				if( ! empty( amp_avia_sc_toggle::$atts['hover_background_color'] ) )
				{
					$hover_styling .= 'background-color: ' . amp_avia_sc_toggle::$atts['hover_background_color'] . '; ';
				}

				if( ! empty( amp_avia_sc_toggle::$atts['hover_font_color'] ) ) 
				{
					$hover_styling .= 'color: ' . amp_avia_sc_toggle::$atts['hover_font_color'] . '; ';
				}
			}
			...
			if( '' == amp_avia_sc_toggle::$atts['faq_markup'] )
			{
			...
			$output .= '        <header data-fake-id="#' . $toggle_atts['custom_id'] . '" class="single_toggle toggler ' . $titleClass . $inherit . '" ' . $markup_title . ' ' . $colors . ' ' . $hover_styling_markup . ' role="tab" tabindex="0" aria-controls="' . $toggle_atts['custom_id'] . '">' . $toggle_atts['title'] . '<span class="toggle_icon" ' . $icon_color . '>';
			$output .= '        <span class="vert_icon"></span><span class="hor_icon"></span></span></header>';
        ...
        function create_tag_string( $tags, $toggle_atts )
        {
            ...
            if( trim( $tags ) != '' )
            {
                ...
                foreach( $tags as $tag )
                {
                    ...
                    {
                        $tag_string .= '{' . $tag . '} ';
                        amp_avia_sc_toggle::$tags[ $tag ] = true;
                    }
                }
            }
        ...
        function sort_list( $toggle_atts )
        {
            ...
            if( ! empty( amp_avia_sc_toggle::$tags ) )
            {
                ksort( amp_avia_sc_toggle::$tags );
                $first_item_text = apply_filters( 'avf_toggle_sort_first_label', __( 'All', 'avia_framework' ), $toggle_atts );
                $start = array( $first_item_text => true );
                amp_avia_sc_toggle::$tags = $start + amp_avia_sc_toggle::$tags;
				
				$sep = apply_filters( 'avf_toggle_sort_seperator', '/', $toggle_atts );
				
                foreach( amp_avia_sc_toggle::$tags as $key => $value )
                {
    ...
    function amp_avia_toggles_css(){
       $toggle_css =   'amp-accordion .single_toggle {
	                        background: white;
						...
						amp-accordion section[expanded] .toggle_icon .vert_icon {
						border-left-width: 0;
						}

						.single_toggle , .toggle_icon, .toggle_icon .vert_icon,  .toggle_icon .hor_icon{
						    border-color: #ebebeb;
						}';               
	  echo $toggle_css;
    }
