<?php
// [ux_banner]
function ux_banner_amp( $atts, $content = null ){
  ...
  $default = wp_get_attachment_image_src( $bg, 'ux' );
  $img_atts = wp_get_attachment_image_src( $bg, $default );
  $img_src = $img_atts[0];
  $bg_style = '';
  if ( (function_exists( 'ampforwp_is_amp_endpoint' ) && ampforwp_is_amp_endpoint())){
    if(!empty($img_src)){
        $bg_style = 'style="background-image:url('.$img_src.')"';  
    }
  }
 $bg_span  = '';
if( (function_exists( 'amp_activate' ) && amp_activate()) && !empty($img_src )){
    $bg_span = '<span class="banner-bg-hide">'.$img_src.'</span>';
}
  ?>

  <div class="banner <?php echo $classes; ?>" id="<?php echo $_id; ?>"<?php echo $bg_style;?>>
     ...
     <div class="banner-inner fill">
      <?php echo $bg_span;?>
        <div class="banner-bg fill" <?php echo $parallax; ?>>
            ...
            <?php 
            require( AMP_PAGEBUILDER_COMPATIBILITY_PLUGIN_DIR."includes/ux-builder/commons/video.php" ) ;?>
            <?php if($bg_overlay) echo '<div class="overlay"></div>' ?>
            <?php require( AMP_PAGEBUILDER_COMPATIBILITY_PLUGIN_DIR."includes/ux-builder/commons/border.php" ) ;?>
...
add_action('amp_post_template_css','amp_banner_custom_css');
function amp_banner_custom_css(){
  echo ".banner {
