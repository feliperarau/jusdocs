...
define('AMPFORWP_LAYOUTS_FILE',__FILE__);
...
if ( ! defined( 'AMP_LAYOUTS_VERSION' ) ) {
    define( 'AMP_LAYOUTS_VERSION', '1.9.37' );
}
   ...
   public $ampforwp_exportDetails = array();
    ...
    public function amplayout_pro_widgets_fields( $sections) {
        ...
        $headeroption = array(
            ...
            'default' => 0,
            'required' => array(
                array('header-type', '=' , array('4','5','6','7','8') ),
                array('customize-options', '=' , '1'),
                ),
        );
        $dateseperator = array(
            'section_id' => 'amp-single',
            'priority'  => '1',
            'class'=> '',
            'id'=> 'ampforwp_layout_date_seperator', 
            "type"=>"switch",
            'title'=>"Date Seperator",
            'default' => 1,
            'required' => array(
                array('single-design-type', '=' , array('2','3') ),
                ),
        );
        $submenucolor = array(
            'section_id'    => 'amp-theme-header-settings',
    ...
    $amplayoutstranslator =    array(
            'section_id' => 'amp-translator',
            'priority'  => '1',
            'subsection' => true,
            'id' => 'amp-layouts-footer-Text',
            'type' => 'section',
            'title' => esc_html__('Amp Layouts Translation Text', 'accelerated-mobile-pages'),
            'indent' => true,
        ...
        $ftrconnecttxt = array(
            ...
            'priority'  => '1',
            'subsection' => true,
            'id' => 'amp-translator-footer-connect-text',
            'type'     => 'text',
            'title'    => esc_html__('Connect', 'accelerated-mobile-pages'),
            'default'  => esc_html__('CONNECT','accelerated-mobile-pages'),
        ...
        foreach ($sections as $key => $section_value) {
            ...
            if($section_value['id']=='amp-translator'){ 
               $sections[$key]['fields'][] = $amplayoutstranslator;
               $sections[$key]['fields'][] = $ftrconnecttxt;
            }
            if($section_value['id']=='amp-single'){
                $i=0;
                foreach ($sections[$key]['fields'] as $insert_key => $value) {
                    $i++;
                    if($value['id'] == 'ampforwp-pagination-select' )
                    {
    ...
    public function swiftOptionsDownload(){
        if ( ! isset( $_GET['secret'] ) || $_GET['secret'] != md5( md5( Redux_Helpers::get_auth_key_secret_key() ) . '-redux_builder_amp' ) ) {
   ...
   $importReduxSettings = $_POST['importSettings'];
    ...
    if(count($importData)>0){
        foreach ($importData as $key => $value) {
            ...
            if(isset($redux_builder_amp[$key]) && class_exists('ReduxCore\\ReduxFramework\\Redux')){              
              \ReduxCore\ReduxFramework\Redux::setOption( "redux_builder_amp", $key, $value);
            }
        }
   }
...
// Add this support
function add_this_for_layouts(){
    $add_this = '';
    if(function_exists('ampforwp_addThis_support')){
     $add_this = ampforwp_addThis_support();
     } 
     return $add_this;
}
...
// Exclude classes of Infinity scroll feature
add_filter("ampforwp_infinite_scroll_exclude_items","layout_infinite_scroll_exclude_items");

function layout_infinite_scroll_exclude_items($classes){
$design = ampforwp_get_setting('amp-design-selector');
if ( 4 == $design ) {
$exclude_classes = array(".ftr");
$classes = array_merge($classes, $exclude_classes);
}
return $classes;
}
...
// activation
/* register_activation_hook( AMPFORWP_LAYOUTS_FILE, 'ampforwp_get_core_settings' );

function ampforwp_get_core_settings(){
    //ampforwp settings backup
 $core_selectedOption = get_option('redux_builder_amp',true);
$ampforwp_options['header-type'] =  $core_selectedOption['header-type'];
$ampforwp_options['single-design-type'] =  $core_selectedOption['single-design-type'];
$ampforwp_options['footer-type'] =  $core_selectedOption['footer-type'];
$ampforwp_options['menu-type'] =  $core_selectedOption['menu-type'];
update_option('ampforwp_backup_options',$ampforwp_options);
     
     //updating layouts back-up settings
 $layouts_selectedOption = get_option('layouts_backup_options',true);
 update_option('redux_builder_amp',$layouts_selectedOption);
}

// deactivation
register_deactivation_hook( AMPFORWP_LAYOUTS_FILE, 'ampforwp_reset_core_settings' );
function ampforwp_reset_core_settings(){

//layouts settings backup
 $layouts_selectedOption = get_option('redux_builder_amp',true);
 update_option('layouts_backup_options',$layouts_selectedOption);

//updating ampforwp back-up settings
 $core_selectedOption = get_option('ampforwp_backup_options',true);

$ampforwp_options['header-type'] =  $core_selectedOption['header-type'];
$ampforwp_options['single-design-type'] = $core_selectedOption['single-design-type'];
$ampforwp_options['footer-type'] =  $core_selectedOption['footer-type'];
$ampforwp_options['menu-type'] =  $core_selectedOption['menu-type'];
 update_option('redux_builder_amp',$ampforwp_options);

}
*/
/*register_activation_hook( __FILE__, 'default_layout_designs' );
function default_layout_designs(){
       global $header,$footer,$design,$menu;
       $redux_builder_amp = get_option('layout_data');
       $redux_initial_data = get_option('redux_builder_amp');
if($redux_initial_data['amp-design-selector'] == 4){
   if(isset($redux_builder_amp)){
       $redux_builder_amp['header-type']  = $redux_builder_amp['header'];
       $redux_builder_amp['menu-type'] = $redux_builder_amp['menu'];
       $redux_builder_amp['single-design-type']  = $redux_builder_amp['single-design'];
       $redux_builder_amp['footer-type'] = $redux_builder_amp['footer'];
       update_option('redux_builder_amp',$redux_builder_amp);
   }else{
       $redux_initial_data['header-type']  = $redux_initial_data['header-type'];
       $redux_initial_data['menu-type'] = $redux_initial_data['menu-type'];
       $redux_initial_data['single-design-type']  = $redux_initial_data['single-design-type'];
       $redux_initial_data['footer-type'] = $redux_initial_data['footer-type'];
       update_option('redux_builder_amp', $redux_initial_data);
   }
}
}

register_deactivation_hook( __FILE__, 'amp_default_layout_designs' );
function amp_default_layout_designs(){
    $redux_builder_amp = get_option('redux_builder_amp');
    $layoutBackup = array();
    $layoutBackup['header'] = $redux_builder_amp['header-type'];
    $layoutBackup['footer'] = $redux_builder_amp['footer-type'];
    $layoutBackup['single-design'] = $redux_builder_amp['single-design-type'];
    $layoutBackup['menu'] = $redux_builder_amp['menu-type'];
    add_option('layout_data', $layoutBackup);
    if($redux_builder_amp['amp-design-selector'] == 4){
       $redux_builder_amp['header-type']  = 1;
       $redux_builder_amp['menu-type'] = 1;
       $redux_builder_amp['single-design-type']  = 1;
       $redux_builder_amp['footer-type'] = 1;
        update_option('redux_builder_amp',$redux_builder_amp);
    }
}*/
register_deactivation_hook( __FILE__, 'amp_default_layout_designs' );
...
$path = plugin_basename( __FILE__ );
    add_action("after_plugin_row_{$path}", function( $plugin_file, $plugin_data, $status ) {
        ...
        $pluginstatus = isset($pluginsDetail['status']) ? $pluginsDetail['status'] : '';
...
// Widget Title Wrapper for AMP Layouts single sidebar
if(function_exists('ampforwp_get_setting')){
    if ( true == ampforwp_get_setting('gnrl-sidebar') &&  true == ampforwp_get_setting('swift-sidebar') && 2 == ampforwp_get_setting('single-design-type') ){
        add_filter( 'dynamic_sidebar_params', 'amplayouts_wrap_widget_titles' );
    } // sidebar condition ends here
}
function amplayouts_wrap_widget_titles( array $params ) {
        $widget =& $params[0];
        $widget['before_title'] = '<div class="wdg-tlt-wrap"><h4>';
        $widget['after_title'] = '</h4></div>';
